...
description: 模块画像 catalog 与业务术语表 glossary 自举（完整流程见 framework/skills/project/catalog-bootstrap/SKILL.md）
---
...
本工程须先完成 [`framework-init`](framework/skills/project/framework-init/SKILL.md)：实例根下已有有效的 `framework.config.json`，且本 skill 将读写的 catalog、glossary、架构说明等 **paths** 与 **`architecture` 段**已由初始化落地或与之一致。未完成 `/framework-init` 前请勿执行本 skill。

**Harness 运行时前置**：执行本 Skill 中任意 `harness-runner` / `npx ts-node harness-runner.ts` 前，须满足 [Host harness readiness · Tier_1](framework/skills/reference/host-harness-readiness.md)。
...
**用户确认 UX**：[user-confirmation-ux.md](framework/skills/reference/user-confirmation-ux.md) · `catalog.staging_module` / `catalog.staging_glossary`。
...
`framework/profiles/<project_profile.name>/skills/catalog-bootstrap/profile-addendum.md`
...
> **动态资产引用**：若正文出现 `` `profile-skill-asset:<skill>/<asset_key>` ``，须按 [Profile skill asset protocol](framework/README.md#profile-skill-asset-protocol) 解析。
...
读取模板（以当前 `project_profile.name` 为准，不存在则仅依赖本 SKILL 与宿主文档）：

```
framework/profiles/<project_profile.name>/skills/catalog-bootstrap/templates/module-card-template.yaml
```
...
字段填写要点（详见 `framework/profiles/<project_profile.name>/skills/catalog-bootstrap/prompts/infer-module-card.md`）：
...
Responsibilities:
  ...
  3. …
NOT_responsible_for:
  1. <不归本模块的事项 1>（归 <SiblingModule>）
  2. <不归本模块的事项 2>（归 <OtherModule>）
  3. …
...
> **领域填充实例（具体模块名与业务词见 profile）**：`` `profile-skill-asset:catalog-bootstrap/examples_domain_mapping` ``。
...
⚠ easily_confused_with:
    + module: <NewConfusee>   disambiguation: "<消歧>"

✓ 其他字段无变化（按实际 diff 列字段名）
...
**只问一个问题**（SSOT：[user-confirmation-ux.md](framework/skills/reference/user-confirmation-ux.md) · registry `catalog.staging_module`）：
...
| `s` / `跳过` / `稍后` | 保留 staging 不动，告知"staging 已留档，下次回来可直接说'继续确认 PendingTaskModule'" |
...
3. **删除**已合并的 staging 文件（`fs.unlink` / `rm` 均可）。审计溯源靠 git 历史——主 catalog 的 commit message 用下列约定方便回查：
   - CREATE 模式：`catalog: add <ModuleName> via catalog-bootstrap`
   - UPDATE 模式：`catalog: refresh <ModuleName> via catalog-bootstrap (<变更摘要>)`<br>&nbsp;&nbsp;&nbsp;&nbsp;其中 `<变更摘要>` 取 Step 5.1.B diff 里动过的字段名列表，如 `one_liner + key_exports + easily_confused_with`
...
- 架构级变更记录表的准入条件和触发时机以 [requirement-design · Step 12](framework/skills/feature/requirement-design/SKILL.md) 为准，事件描述格式：`| YYYY-MM-DD | module_set_change \| responsibility_rewrite | <具体变化> |`。
- 若 catalog 变化是由一个具体 feature 的 design 驱动的，且该 design 已经声明了 `architecture_impact != none`，则在 requirement-design · Step 12 分支中统一处理，不要在本 Skill 里重复追加。
...
# 由 catalog-bootstrap · Phase B (`/glossary-bootstrap`) 逐条追加
# AI 绝不直接写入本文件，除非拿到用户在对话里的 `y` 确认
...
# ============================================================================
# 作用：供 catalog-bootstrap Phase B 用来反向查 canonical_module。每条会在对话里
...
#   4) 特别要收录"意思相近但字面不同"的词（例如：控制台 / 管理后台 / 运营入口
...
- **允许** 在用户填完后做一次"可疑漏项"提示（例："你填了'工作台'但没填'草稿箱'，要不要也加上？"），但不替用户决定

### Step 2. 逐条术语提出映射建议（一次一条或一批）

对每个术语 `<T>`，读 `doc/module-catalog.yaml`，执行 `framework/profiles/<project_profile.name>/skills/catalog-bootstrap/prompts/infer-glossary-term.md` 的 Step 1 → Step 6，核心骨架：
...
> Step 0（输入预检）、Step 1.5、Step 2.5、Step 4.5 均是**弱模型护栏**，不可省略。详细规则见 `framework/profiles/<project_profile.name>/skills/catalog-bootstrap/prompts/infer-glossary-term.md`。
...
模板见 `framework/profiles/<project_profile.name>/skills/catalog-bootstrap/templates/glossary-term-template.yaml`。
...
已落 15 条 staging 到 doc/glossary-staging/：
  high: 9 条    medium: 4 条    low: 2 条（"TypeAExport"、"归档副本"）
...
───────────────────────────────────
匹配置信度: high
Canonical module: <FeatureModule> (<Layer>)
匹配依据: typical_business_terms[k] = "示例术语"
Aliases: []

⚠️ 易混项（必读，Scope 守门入口）：
  • <ModuleA> (<ModuleId>) — <一句话消歧>
...
> **领域填充实例**：`` `profile-skill-asset:catalog-bootstrap/examples_domain_mapping` ``。
...
2. **删除**已合并的 staging 文件。审计溯源靠 git 历史——commit message 建议带 `via catalog-bootstrap from <term>.yaml`。
...
- 模块画像推断 prompt：`framework/profiles/<project_profile.name>/skills/catalog-bootstrap/prompts/infer-module-card.md`
- 术语映射推断 prompt：`framework/profiles/<project_profile.name>/skills/catalog-bootstrap/prompts/infer-glossary-term.md`
...
| 消费者 | 消费的产出 | 用途 |
|--------|-----------|------|
| **prd-design (PRD)** Step 1.5 | catalog + glossary | 术语消歧、Scope 声明校验 |
| **requirement-design (Design)** Step 2.5 | catalog | Scope 扩展提议时查候选模块 |
