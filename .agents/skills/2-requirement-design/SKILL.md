...
> **用户确认 UX**：[user-confirmation-ux.md](../reference/user-confirmation-ux.md) · `design.scope_expansion` / `design.ok_to_code` / `design.arch_impact` / `design.split_table` / `phase.next_step`。
...
| 输入项 | 必需 | 说明 |
|--------|------|------|
| PRD.md | ✅ | 对应功能的 PRD 文档（Skill 1 输出），路径通常为 `doc/features/{module}/prd/PRD.md` |
...
四项全部通过后，设计阶段完成，**具备**进入 Skill 3（编码）的**资格**；**不授权**自动开 Skill 3。
...
- **上游输入**:
  - `doc/features/{module}/prd/PRD.md`（Skill 1 输出）
