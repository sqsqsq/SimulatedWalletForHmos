...
> 宿主可见能力仅 **slash / 自然语言 / Skill**；**禁止**要求宿主手跑 harness（执行权见 [user-confirmation-ux.md](framework/skills/reference/user-confirmation-ux.md) §8.2b · `framework-agent-execution`）。
...
1. [host-harness-readiness](framework/skills/reference/host-harness-readiness.md) Tier_1 + [harness-cli-cwd](framework/skills/reference/harness-cli-cwd.md)。
2. **Personal setup**（按是否显式指定 adapter 分流）：
   ...
   **B. 用户未显式指定 adapter** → **BLOCKER**：启动 goal-runner **之前**须 [personal-setup-gate](framework/skills/reference/personal-setup-gate.md)：
