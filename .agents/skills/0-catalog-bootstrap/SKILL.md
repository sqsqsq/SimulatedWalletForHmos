...
```
framework/profiles/<project_profile.name>/skills/0-catalog-bootstrap/templates/module-card-template.yaml
```
...
字段填写要点（详见 `framework/profiles/<project_profile.name>/skills/0-catalog-bootstrap/prompts/infer-module-card.md`）：
...
> **领域填充实例（具体模块名与业务词见 profile）**：`` `profile-skill-asset:0-catalog-bootstrap/examples_domain_mapping` ``。
...
对每个术语 `<T>`，读 `doc/module-catalog.yaml`，执行 `framework/profiles/<project_profile.name>/skills/0-catalog-bootstrap/prompts/infer-glossary-term.md` 的 Step 1 → Step 6，核心骨架：
...
> Step 0（输入预检）、Step 1.5、Step 2.5、Step 4.5 均是**弱模型护栏**，不可省略。详细规则见 `framework/profiles/<project_profile.name>/skills/0-catalog-bootstrap/prompts/infer-glossary-term.md`。
...
模板见 `framework/profiles/<project_profile.name>/skills/0-catalog-bootstrap/templates/glossary-term-template.yaml`。
...
- 模块画像模板：`framework/profiles/<project_profile.name>/skills/0-catalog-bootstrap/templates/module-card-template.yaml`
- 术语条目模板：`framework/profiles/<project_profile.name>/skills/0-catalog-bootstrap/templates/glossary-term-template.yaml`
- 模块画像推断 prompt：`framework/profiles/<project_profile.name>/skills/0-catalog-bootstrap/prompts/infer-module-card.md`
- 术语映射推断 prompt：`framework/profiles/<project_profile.name>/skills/0-catalog-bootstrap/prompts/infer-glossary-term.md`
