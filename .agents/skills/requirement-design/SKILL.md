...
description: PRD → 技术设计 design.md（完整流程见 framework/skills/feature/requirement-design/SKILL.md）
---

# 需求设计 Skill (`requirement-design`)

> **用户确认 UX**：[user-confirmation-ux.md](framework/skills/reference/user-confirmation-ux.md) · `design.scope_expansion` / `design.ok_to_code` / `design.arch_impact` / `design.split_table` / `phase.next_step`。

## 前置（依赖初始化 Skill 产物）

本工程须先完成 [`framework-init`](framework/skills/project/framework-init/SKILL.md)：实例根下已有有效的 `framework.config.json`，且本 skill 与 harness 所依赖的 **paths** 及 **`architecture` 段**已由初始化写入或与之一致。未完成 `/framework-init` 前请勿执行本 skill。

**Harness 运行时前置**：执行本 Skill 中任意 `harness-runner` / `npx ts-node harness-runner.ts` / `check-receipt.ts`（依赖 harness npm）前，须满足 [Host harness readiness · Tier_1](framework/skills/reference/host-harness-readiness.md) 与 [Shell cwd 契约](framework/skills/reference/harness-cli-cwd.md)（harness 之后用 `cd framework/harness && npx ts-node scripts/check-receipt.ts`）。
...
`framework/profiles/<project_profile.name>/skills/requirement-design/profile-addendum.md`

其中 `<project_profile.name>` 取自 `framework.config.json > project_profile.name`（未声明时由 harness 按仓库指纹回落默认 profile，见 [framework/harness/config.ts](framework/harness/config.ts) 与 init Skill S2.1（`project_profile`））。若该文件不存在，则仅依赖本 SKILL 正文 + 对应 profile 下模板/示例路径。

> **Agent 行为规约（BLOCKER）**：完整阅读 [`agent-behavioral-principles.md`](framework/skills/reference/agent-behavioral-principles.md)。**Research Sub-Phase 完成前禁止进入 Step 2.5（Scope 冻结）及后续设计撰写。**

> **动态资产引用**：正文中的 `` `profile-skill-asset:<skill>/<asset_key>` `` 须按 [Profile skill asset protocol](framework/README.md#profile-skill-asset-protocol) 解析。
...
本 Skill 是项目全生命周期流水线的**第二环**。上游输入来自 prd-design（PRD 设计）的 `PRD.md`，输出（design.md）将流入 coding（编码）。
...
| 输入项 | 必需 | 说明 |
|--------|------|------|
| PRD.md | ✅ | 对应功能的 PRD 文档（prd-design 输出），路径通常为 `doc/features/{module}/prd/PRD.md` |
...
**若缺少 PRD.md**：提示用户先运行 prd-design 生成 PRD 文档。
...
1. **请求路由**：用户仅表达「修订 design」「更新技术设计」「改 contracts / Spec」「修复设计段落」「对齐 PRD 调整设计」等，而**未**在同一条或明确承接的指令中同时要求编码（见下条），则**只激活本 Skill**，本轮定性为 **design 迭代**，不是流水线自动滑入 coding。
2. **何谓同时要求编码（示例，非穷举）**：同一条消息里出现「开始编码」「按现行 design 实现 / 落地」「写代码 / 开发」且指向当前 `doc/features/<feature>/`，可视为用户显式授权连续进入 coding。仅有「把设计改对」「补充设计」**不算**编码授权。
...
4. **落盘 → harness → 停等人审（产品闸门）**：每次 `design.md` + Step 11 的 Spec 写入磁盘后，须**立即**进入 Step 13.1 跑 `harness-runner --phase design`（见 Step 13），**禁止**「先改实现再补 design.harness」。Step 13.1 通过后，向用户交付 harness 摘要与设计变更要点；**每次**（含首遍 design 与 design 修订）须 **`design.ok_to_code` 编号确认**：`1=design OK，可编码` / `2=继续改 design`——**不得**在同一轮 agent 执行流里「写完设计侧 SSOT → 立刻改实现层产物」直连。
5. **历史阶段不免除**：即使用户过去已跑通 coding / review / UT，只要**本轮改动了** `design.md` 或 `contracts.yaml`（等设计侧 SSOT），设计审阅与「明示编码」流程**重新适用**；不得以「以前实现过」为由跳过。

### Step 1: 读取并分析 PRD

1. 读取指定的 `doc/features/{module}/prd/PRD.md` 文件
2. 提取以下关键信息：
...
1. 读取 `doc/features/{module}/prd/PRD.md` 的 **「Scope 声明」** 章节中的 yaml 代码块。
...
> 真正的业务编排代码（Coordinator / Flow / Page 命名方法 / 导出函数）由 coding 选择最贴合复杂度的形式落地。
...
若本 feature **全部条件都不满足**（例：首页一次性加载、单接口查询、单按钮跳转），**不要**产出 `use-cases.yaml`。business-ut 会走退化模式，基于 `acceptance.yaml` + `dag.yaml` 直接针对 data 层函数写 UT。

#### 6.2 若决定产出 `use-cases.yaml`

产出以下两份文档（代码形态仍由 coding 决定）：

1. **design.md 新增「业务流程 UseCase 清单」章节**（见 `` `profile-skill-asset:requirement-design/design_template` `` 的 `## 六、业务流程 UseCase 清单`），含：
...
2. **`doc/features/{module}/use-cases.yaml`**（Spec 文件，与 `contracts.yaml` 同目录）：
   - Schema 见 `` `profile-skill-asset:business-ut/use_cases_schema` ``
   - 规范级样例目录见 `` `profile-skill-asset:business-ut/sample_flow_dir` ``
   ...
   - `coordinator` 只写**符号名**（类名 / `Page.method` / 导出函数名），指向 coding 将要实现（或已存在）的真实代码；**不**强制放在 `domain/usecase/` 下
   ...
   - `branches[].linked_acceptance` 至少关联一条 AC/BD；business-ut 会按此清单 1:1 生成 UT 用例

#### 6.3 **禁止**的反模式（v2.1 明确约束）

- ❌ 在 design / use-cases.yaml 里要求"必须在 `domain/usecase/` 下新建 `XxxUseCase` 类"——代码形态由 coding 决定
- ❌ 为某个 data 层类额外套一层 `XxxPort` 接口**只为了 UT 注入方便**——直接引用既有 data 层类即可，UT 用 Spy/Fake 子类化或原型替换
- ❌ 把 **路由/弹窗/Toast 等 UI 运行时能力**登记为 `data_boundaries`——UI 副作用走 `ui_subscription`（design 文档化）与 prd-design 的 `acceptance.yaml` > `device_focus`，不进 UT；具体禁入符号见宿主 profile addendum
- ❌ 对业务逻辑非常简单的 feature 硬凑 `use-cases.yaml`（如首页一次性加载）——应直接让 business-ut 基于 data 层函数 + dag 写 UT

### Step 7: 设计展示层

1. **页面组件树**：每个页面拆分为哪些自定义组件（**示意树与宿主页面容器术语**见 `framework/profiles/<project_profile>/skills/requirement-design/profile-addendum.md`）
   ```
   <HomePage>
   ├── <SectionA>（复杂组件）
   │   └── <Widget>（基础组件）
...
> **顺序纠错**：必须把 `design.md` 写入磁盘后，脚本 harness（Step 13.1）才能读取。**「用户确认」指是否冻结设计并授权进入 coding 写源码**，不是「确认后才允许写入 design.md」——否则会出现「没落盘就跑不了 harness」与「先把代码写完再补 harness」的违规路径。

1. 将（更新后的）设计文档写入：
   ```
   doc/features/{module-name}/design/design.md
   ```
...
4. **立即进入 Step 13**：归档落盘后**不得**先做编码；必须先跑 Step 13（至少 13.1，零 BLOCKER），再与用户对齐「可否进入 coding」。若本条与「设计与编码的会话内硬边界」冲突，**以更严格的停等规则为准**。

### Step 11: 提取功能级 Spec

设计文档归档后，**必须**同步提取功能级接口契约到 `doc/features/{module-name}/` 目录。Spec 是连接生成层和验证层的枢纽，也是 coding（编码）的强契约基准。
...
> **UT / mock-plan 门禁（v2.3）**：`interfaces[].methods` 中 **`params` 须含完整类型文本**、`return` 必须填写准确返回类型（含 `Promise<...>`）——下游 business-ut 的 `ut/mock-plan.yaml` 与 harness `ut_mock_plan_contracts_consistent` 依赖此信息生成可用的类型化 Test Double。
...
#### 11.2 补充边界用例 Spec（若 prd-design 未产出）

若 `doc/features/{module-name}/acceptance.yaml` 已由 prd-design 产出，检查并补充设计阶段新增的边界场景（如从技术角度发现的新边界用例）。

若 prd-design 未产出 `acceptance.yaml`（历史原因），则从 PRD.md 中提取并创建。

#### 11.3 （条件式）提取业务流程 UseCase 清单 (`use-cases.yaml`)

**仅当** Step 6.1 的复杂度阈值命中时才产出 `doc/features/{module-name}/use-cases.yaml`；不满足阈值的 feature**不写**该文件，交由 business-ut 基于 `acceptance.yaml` + `dag.yaml` 直接针对 data 层写 UT。

若需要产出：

- Schema：见 `` `profile-skill-asset:business-ut/use_cases_schema` ``
- 参考样例：`` `profile-skill-asset:business-ut/sample_flow_use_cases` ``
- 关键强约束：
  - `use_cases[].coordinator` 仅声明**符号名**（类名 / `Page.method` / 导出函数），指向 coding 之后会实现（或已存在）的真实代码；`coordinator_file` 可选，不强制放在 `domain/usecase/`
  - `ui_bindings[].user_actions[].calls` 必须是命名函数（coding harness 会校验 `named_business_handler`）
  ...
  - 若某条分支找不到对应 AC/BD，回到 prd-design 补充后再回填
...
参考已有示例：`doc/features/<your-feature>/contracts.yaml`（简单 feature，无 use-cases.yaml）、多步流程可参考 `` `profile-skill-asset:business-ut/sample_flow_use_cases` ``。

> **为什么这一步如此重要**：`contracts.yaml` 是 coding 编码时的强契约——文件路径、接口签名、组件 Props 必须与 contracts.yaml 一致；`use-cases.yaml`（若存在）是 business-ut 业务级 UT 的蓝图——DAG 与 UT 用例按 branches 1:1 生成。Harness 也依赖它们做接口一致性验证与 UT 覆盖追溯。未达阈值的简单 feature 不产出 `use-cases.yaml`，避免为测试工具人为引入架构复杂度。
...
在 design.md 的 `## Scope 声明与继承` 章节下，补齐（或确认）`### 架构影响声明 (architecture_impact)` 子节。模板与字段见 `` `profile-skill-asset:requirement-design/design_template` ``：
...
- 更新 [doc/module-catalog.yaml](../../../doc/module-catalog.yaml)：新增 / 删除 / 迁层对应模块条目，见 [catalog-bootstrap](framework/skills/project/catalog-bootstrap/SKILL.md) Phase A 的增量流程。
...
> 下文「物理拦截层」：**部分 adapter** 经 framework-init 在实例根下发 **Stop hook**，在消息结束前读取 state 并阻断「假完成」（Layer 3 行为与路径见 [framework/agents/README.md](framework/agents/README.md)）。**未**配置该能力的 adapter 不设物理层豁免，仍须满足 Layer 1（全局入口 §6.5「反假设条款」）+ Layer 2（完成回执 + `check-receipt.ts`）——**没有 Stop hook ≠ 豁免 BLOCKER**，少跑一项即任务失败。
...
4. 完成回执 `doc/features/<feature>/design/phase-completion-receipt.md` 已填写并通过 `cd framework/harness && npx ts-node scripts/check-receipt.ts --feature <feature> --phase design` 校验。
...
四项全部通过后，设计阶段完成，**具备**进入 coding（编码）的**资格**；**不授权**自动开 coding。

**闭环停等（BLOCKER，user-confirmation-ux §8）**：四件套 PASS 后须 **`design.ok_to_code`** 或 **`phase.next_step`** 停等（除非 batch 授权 §8.2）。物理拦截层会读 `framework/harness/state/.current-phase.json` 与上述四份凭证决定能否放行。
...
| 产出 | 路径 |
|------|------|
| 设计文档 | `doc/features/{module-name}/design/design.md` |
...
`` `profile-skill-asset:requirement-design/design_template` ``
...
数据模型和接口规范可参考以下模板：
- 数据模型模板: `` `profile-skill-asset:requirement-design/data_model_template` ``
- 接口规范模板: `` `profile-skill-asset:requirement-design/api_spec_template` ``
...
- 设计文档模板: `` `profile-skill-asset:requirement-design/design_template` ``
- 接口规范模板: `` `profile-skill-asset:requirement-design/api_spec_template` ``
- 数据模型模板: `` `profile-skill-asset:requirement-design/data_model_template` ``
- 示例设计文档: `` `profile-skill-asset:requirement-design/example_design` ``
...
- **上游输入**:
  - `doc/features/{module}/prd/PRD.md`（prd-design 输出）
  - `doc/features/{module}/acceptance.yaml`（prd-design 产出的验收标准 Spec）
  - `doc/architecture.md`（项目架构全貌，跨 Skill 共享）
- **下游消费者**:

| 消费者 | 消费的产出 | 用途 |
|--------|-----------|------|
| **coding (编码)** | design.md + contracts.yaml | 按文件规划和接口契约逐模块生成代码 |
| **code-review (Code Review)** | design.md + contracts.yaml | 对照检查实现一致性 |
| **business-ut (业务级 UT)** | design.md + contracts.yaml | 读取业务流程信息生成 DAG |
| **Harness (验证层)** | contracts.yaml | 脚本/AI 验证代码接口一致性和文件完整性 |

## 约束与注意事项

1. **PRD 是唯一的需求来源**：不得自行添加 PRD 中未提及的功能，若发现 PRD 缺失重要场景，应标注并建议用户回到 prd-design 补充
...
5. **设计即契约**：design.md 中的接口签名、文件路径、组件 Props 定义将作为 coding 编码的强契约，务必精确
...
9. **跨模块拆分是核心能力**：requirement-design 必须把功能点放进**正确的模块 + 合法外层**，保证职责单一、依赖与 DSL 一致
