...
| 模块内四层分层 | `specs/phase-rules/coding-rules.yaml` | BLOCKER |
| 接口一致性 | `specs/features/{module}/contracts.yaml` | BLOCKER |
| 文件完整性 | `specs/features/{module}/contracts.yaml` | BLOCKER |
| 资源引用完整性 | `specs/phase-rules/coding-rules.yaml` | BLOCKER |
| 命名规范 | `specs/phase-rules/coding-rules.yaml` | MAJOR |
| 硬编码字符串 | `specs/phase-rules/coding-rules.yaml` | MAJOR |
| 异常处理 | `specs/features/{module}/acceptance.yaml` | MAJOR |
| 业务逻辑正确性 | `doc/features/{module}/design.md` | MAJOR |
| 数据所有权 | `specs/phase-rules/coding-rules.yaml` | MAJOR |
| 模拟数据隔离 | `specs/phase-rules/coding-rules.yaml` | MINOR |
...
| coding-rules.yaml | ✅ | 编码阶段规约，路径 `specs/phase-rules/coding-rules.yaml` |
...
2. 读取以下文件：
   ...
   - `specs/phase-rules/coding-rules.yaml` — 编码阶段规约
...
```
skills/4-code-review/templates/review-report-template.md
```
...
- `specs/phase-rules/review-rules.yaml` — 阶段级通用规则（章节存在性、表格格式、严重程度值域等）
...
- **Prompt 模板**：`harness/prompts/verify-review.md`
...
- 阶段级规约: `specs/phase-rules/review-rules.yaml`
- 编码规约参考: `specs/phase-rules/coding-rules.yaml`
- 脚本 Harness: `harness/scripts/check-review.ts`
- AI Harness Prompt: `harness/prompts/verify-review.md`
