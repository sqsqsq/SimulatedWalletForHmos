...
| B. 在对应 `NOT_responsible_for[i]` 原文里加消歧规则 | 把原文改成"X 指 A 时属本模块；指 B 时不属（详述 B 属何模块）" | 同词多义（如"账户" = 华为登录身份 vs = 钱包业务账户） |
...
严格遵守 `skills/0-catalog-bootstrap/templates/module-card-template.yaml` 的结构。
