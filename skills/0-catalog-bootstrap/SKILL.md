...
#
# 规模建议：真实钱包工程一般 50-150 条；沙盒 10-30 条足够验证流程
# ----------------------------------------------------------------------------
...
模板见 `skills/0-catalog-bootstrap/templates/glossary-term-template.yaml`。
...
### catalog 阶段检查项（完整列表见 `specs/phase-rules/catalog-rules.yaml`）

- **Structure**：schema_version 存在；每条 module 必填字段完整；layer 值合法；format ∈ {HAP, HAR}；name 不重复
- **Traceability**：`easily_confused_with.module` 指向的模块必须在本文件内存在；`entry_file` 路径在磁盘上真实存在（WARN）

### glossary 阶段检查项（完整列表见 `specs/phase-rules/glossary-rules.yaml`）
...
- Catalog 阶段规约：`specs/phase-rules/catalog-rules.yaml`
- Glossary 阶段规约：`specs/phase-rules/glossary-rules.yaml`
- Catalog 检查脚本：`harness/scripts/check-catalog.ts`
- Glossary 检查脚本：`harness/scripts/check-glossary.ts`
...
```
harness/reports/_catalog/<timestamp>/<model>-catalog/trace.json
harness/reports/_glossary/<timestamp>/<model>-glossary/trace.json
```
