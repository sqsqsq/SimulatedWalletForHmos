...
def build_auto_topup_meeting(out_dir: Path) -> Path:
    """RR v1.0 归档之后开的一次澄清会的语音转写——需求变化只留在这场会里。

    形态照真实转写：会名、时间、参会人（只有姓名，没有角色），按议题分节，
    议题内逐条「姓名 时间：发言」。刻意保留转写件本来就有的毛病：错字（孙好、门线、卡运、面密、三四）、
    说话人改口（300 改回 200）、议题内跳回前面的话题、没收敛就挂起的两件事、
    一个不属于本需求的议题（RR90007）和一个归属判不准的议题。
    各话题该有的去向在 `cases/auto-topup/meeting-answer-key.md`，只进测试域。
    """
    speeches = {
        "议题一：交通卡自动充值（AR90006）": [
            "王磊 14:02：先过自动充值。RR 是上周归档的 v1.0，今天主要把开发那边提的几个问题定掉，SR 那边陈静在。",
            "孙好 14:03：我先说改门线。SR 流程图里写了卡运更新签约，但接口只有四个，create 碰到已有签约是直接返回原来的 contractNo，改不了。",
            "陈静 14:04：对，这个是 SR 漏了。我们原来想的是端侧先 cancel 再 create。",
            "孙浩 14:05：先解约再签的话中间有空窗，刚好那几分钟余额掉下去就不会充，而且又要拉一次面密验证。",
            "陈静 14:06：那卡云这边加一个 updateAutoTopupContract，入参 contractNo、门限、面额，原签约关系不断。",
            "王磊 14:06：行，就加 update，不走解约重签。",
            "刘洋 14:07：有个问题，面额是跟免密单笔额度绑的。面额往上调，额度也跟着变大，按支付的规矩要用户重新验证一次。往下调应该不用，这个我回去跟风控确认。",
            "王磊 14:08：那调高要不要重新验证，等刘洋这边的结论，先别写死。",
            "周婷 14:09：顺便问一下 AC-R1，写的是未实名用户看不到入口，但第五节情形表是引导去实名、回来接着签，这两个测哪个？",
            "王磊 14:10：以情形表为准，入口能看到，点进去引导实名。AC-R1 那句是 v0.3 留下来的，没改掉。",
            "周婷 14:10：那 AC-R1 我按「未实名用户点入口被引导去实名，完成实名前签不成」来测。",
            "王磊 14:11：可以。",
            "周婷 14:12：连续失败停用那条，三四扣款失败就停，失败原因在管理页能看到，这个没问题。",
            "赵敏 14:13：运营这边，开关关了以后，已经签了的人还能不能进管理页？现在只写了照常扣款。",
            "王磊 14:14：开关只管新签约。已经签约的，记录、改档位、关掉自动充值都照常，入口对他们不隐藏。",
            "孙浩 14:15：那端侧入口的显示条件要改，已签约用户开关关了也要显示，点进去直接是管理页。",
            "王磊 14:15：对。",
            "孙浩 14:16：回到刚才改门限那个，只改门限、不动面额，肯定不用重新验证吧？",
            "刘洋 14:16：只改门限不涉及额度，不用验证。",
            "赵敏 14:18：单日上限我们想调到 300，节假日出行多的用户老是被 200 卡住。",
            "王磊 14:19：300 是明年的规划，本期还是 200，别动。",
            "赵敏 14:19：好，那就 200。不过协议文案里要把单日上限写给用户看，不然用户以为没上限。",
            "王磊 14:20：加上，协议文案里写明单日最多充 200 元。",
            "陈静 14:22：还有一个异常，用户在支付那边验证通过了，钱包去调 createAutoTopupContract 失败或者超时。协议号卡云这边十分钟内可以复用，钱包拿同一个协议号重试就行。",
            "刘洋 14:23：支付侧协议号的有效期我印象里不是十分钟，好像是跟验证会话走的，会话一关就失效了，我得查一下。",
            "王磊 14:24：那这个先挂着，你们两边对齐了再定，现在别按十分钟写。",
            "孙浩 14:24：那在这之前端侧怎么处理，失败了让用户重新走一遍验证？",
            "王磊 14:25：等他们结论。",
        ],
        "议题二：交通卡余额提醒（RR90007）": [
            "赵敏 14:30：余额提醒这边，余额低于多少推一条提醒。能不能直接用自动充值那套门限档位？",
            "王磊 14:31：不共用。开了自动充值的人不需要提醒，没开的人档位单独配，提醒最低给到 1 元。",
            "陈静 14:32：那卡云余额上报后两边各判各的，互不影响。",
            "赵敏 14:33：推送文案下周给。",
            "王磊 14:33：好，这个单子下次单独过。",
        ],
        "议题三：交通卡充值记录": [
            "赵敏 14:40：客诉核对要按月份筛选充值记录，最好还能导出。",
            "孙浩 14:41：说的是自动充值管理页里的记录，还是交通卡详情页那个交易记录？两边都有充值。",
            "赵敏 14:42：我们主要是看自动充值的扣款。",
            "王磊 14:43：这个还没走需求流程，挂在哪个单子上我回去跟产品组商量，今天不定。",
            "周婷 14:44：导出要不要做？",
            "王磊 14:44：导出本期肯定不做。",
        ],
    }
    blocks: list = [
        ("h", 1, "交通卡相关需求澄清会（第 2 次）"),
        ("p", "会议时间：2026-09-03 14:00-15:10"),
        ("p", "参会人：王磊、陈静、刘洋、赵敏、孙浩、周婷"),
    ]
    for topic, lines in speeches.items():
        blocks.append(("h", 2, topic))
        blocks.extend(("p", line) for line in lines)
    path = out_dir / "交通卡澄清会记录-0903.docx"
    write_docx(path, blocks, {})
    return path
...
BUILDERS = {
    ...
    "auto-topup": lambda d: [build_auto_topup(d), build_auto_topup_meeting(d)],
}
