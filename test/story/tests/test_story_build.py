...
class TestVerdicts(StoryBuildCase):
    ...
    def test_author_placement_without_verdict_file_is_named(self) -> None:
        ...
        真正做过的是**报错涨到上千之后把裁决件整份删掉**，那时删掉它反而让
        报错数下去了。存在性判把这条逃生口关上，报错文案要说清删不是修。
        """
        self.init_audit()
        self.hand_to_author()
...
class TestImageAndDiagramStates(StoryBuildCase):
    """图类单元的第四态：`material_only`——留在材料、不进 story。

    story 是按叙述逻辑组织的评审载体，不是 PRD 的复刻件。上一版在这里核
    「story 的图数不少于材料」：三张图的材料上它等价于「丢图＝丢内容」，
    三十张图的材料上它等价于逼作者把 PRD 重写一遍。图与文字事实在这件事上
    不对称——一张图不引用，读者还能顺材料清单的链接看原件；一条文字事实
    不进 story 就是丢了。所以「不进 story」只对图类是合法状态。
    """

    def seed_diagram(self) -> None:
        """给夹具材料加一张图 —— 基线夹具里没有图类单元，这一组判据够不着。

        加在材料侧（不是 story 侧）：图类单元是从**来源材料**枚举出来的，
        story 引不引用它正是本组要判的那件事。加完走一遍 init/audit/settle，
        基线仍是通过态，用例再去翻某一条。
        """
        prd = self.root / "doc" / "features" / FEATURE / "RR" / "prd.md"
        prd.write_text(prd.read_text(encoding="utf-8")
    ...
    def image_units(self) -> list:
        return [u for u in self.units if u.get("kind") in ("image", "diagram")]

    def mark_only(self, key: str, value) -> None:
        """把这条记录改成只有 material_only 一态（value 为 None 则清成三态皆空）。"""
        data = self.audit
        for record in data["records"]:
            if record["key"] == key:
                record.clear()
                ...
                if value is not None:
                    record["material_only"] = value
                break
        else:
            self.fail("审计记录里没有 " + key)
        self.write_audit(data)

    def test_a_diagram_left_in_the_material_is_accepted(self) -> None:
        """标了 material_only 并写了理由的图，不再报「没有落点」。"""
        self.seed_diagram()
        units = [u for u in self.image_units() if u["kind"] == "diagram"]
        self.assertTrue(units, "夹具里应当有 diagram 单元")
        self.mark_only(units[0]["key"], "界面细节图，叙述由功能说明章的文字承载")
        _, out = self.check_output()
        self.assertNotIn("没有落点", out)
        self.assertNotIn("不是图类单元", out)

    def test_a_diagram_with_no_state_at_all_is_still_named(self) -> None:
        """三态皆空照旧拦——放宽的是「可以不引」，不是「可以不表态」。"""
        self.seed_diagram()
        units = [u for u in self.image_units() if u["kind"] == "diagram"]
        self.assertTrue(units)
        self.mark_only(units[0]["key"], None)
        self.assert_check_names("也没说明为什么不进")

    def test_material_only_needs_a_reason(self) -> None:
        """空理由分不清「判过了不需要」与「懒得引」。"""
        self.seed_diagram()
        units = self.image_units()
        self.assertTrue(units)
        self.mark_only(units[0]["key"], "")
        self.assert_check_names("没有任何落点")

    def test_material_only_is_refused_on_a_text_unit(self) -> None:
        """文字事实没有「在材料里看」这条路——它不进 story 就是丢了。"""
        self.seed_diagram()
        text_unit = next(u for u in self.units
                         if u.get("kind") not in ("image", "diagram")
                         and not u.get("machine_facing"))
        self.mark_only(text_unit["key"], "留在材料里")
        self.assert_check_names("不是图类单元")

    def test_no_image_count_judgement_exists(self) -> None:
        """数量判据退场要在源码里成立——留着一条，material_only 通道就是摆设。"""
        source = (REPO_ROOT / "doc/extensions/skills/story/scripts/story-build.mjs"
                  ).read_text(encoding="utf-8")
        for gone in ("数量不该少于源", "story 里只引用了"):
            self.assertNotIn(gone, source, "「" + gone + "」还在——逼引的那条判据没退场")


class TestIdentifierTokensGoToTheAppendix(StoryBuildCase):
    """工程标识按附录核，不按叙事落点章核。

    语言红线不许主叙事出现工程标识，而守恒要求 token 在落点核得到——同一个单元
    被两条判据夹死：写进正文违反红线，不写违反守恒，作者怎么写都是错的。
    材料里含标识符的单元恰好都归了附录时两条碰不上；材料把标识符写进业务叙述句
    就碰上了。判定式两处共用一个，作用域也对齐。
    """

    def test_the_identifier_shape_has_one_definition(self) -> None:
        """两处各写一份判定式时，只要有一个 token 两边认得不一样，夹死就会回来。"""
        source = (REPO_ROOT / "doc/extensions/skills/story/scripts/story-build.mjs"
                  ).read_text(encoding="utf-8")
        shape = "/^[A-Za-z][A-Za-z0-9_]{3,}$/"
        self.assertEqual(source.count(shape), 1,
                         "标识符形态在 story-build.mjs 里出现了不止一处定义")
        self.assertIn("isEngineeringIdentifier(t, own)", source)

    def test_a_fenced_identifier_still_counts_in_its_own_chapter(self) -> None:
        """围栏里红线管不到，守恒也就不该把围栏里的词赶去附录。

        图的类型词（围栏首行那个 `flowchart` 之类）只在围栏里出现。按「主叙事不许有
        工程标识」把它赶去附录，是拿一条不管这里的判据管这里。
        """
        source = (REPO_ROOT / "doc/extensions/skills/story/scripts/story-build.mjs"
                  ).read_text(encoding="utf-8")
        self.assertIn("fencedText(chapter).includes(t)", source)

    def test_the_fixture_still_passes(self) -> None:
        """改道不许拦既有良品：夹具的标识符都在附录，正文不写它们照旧过。"""
        self.init_audit()
        self.settle()
        code, out = self.check_output()
        self.assertEqual(code, 0, "夹具本该是过的：" + out)


class TestLedgerExistenceIsTheFirstGate(StoryBuildCase):
    """五件台账缺任一即 BLOCKER——把「删掉它，报错数就下去了」这条路堵死。

    冻结只挡登记之后改台账，挡不住登记之前把台账删掉。而删掉是有动力的：
    ...
    def test_each_missing_ledger_is_named(self) -> None:
        for name in self.LEDGERS:
            with self.subTest(ledger=name):
                ...
                out = self.assert_check_names(name)
                self.assertIn("不是把同伴文件删掉", out)
                self.setUp()          # 下一个子用例要一份干净工作区

    def test_all_five_present_reaches_the_real_judgements(self) -> None:
        self.init_audit()
        self.settle()
        code, out = self.check_output()
        self.assertEqual(code, 0, "五件齐备时应当过：" + out)
...
class TestCheckOutputAggregates(StoryBuildCase):
    """报错洪水按判据类聚合，全量落文件——判定逻辑一个字节不变。

    一千多条逐条刷屏，终端里翻不到头，也看不出错在哪一类；洪水本身就是删台账的动力。
    """

    def flood(self) -> None:
        """把每一章的正文换掉，制造成片的落点核不住。"""
        text = self.story()
        out, skip = [], False
        for line in text.split("\n"):
            if line.startswith("## "):
            ...
            if skip and line.strip():
                out.append("此处内容与材料无关。")
                skip = False
                continue
            out.append(line)
        self.story_path.write_text("\n".join(out), encoding="utf-8")

    def test_a_small_number_of_problems_is_still_listed_one_by_one(self) -> None:
        """聚合是给洪水用的：十条以内摊开更好读。"""
        self.init_audit()
        self.settle()
        (self.src / "copyedit.md").write_text("只有一行\n", encoding="utf-8")
        code, out = self.check_output()
        self.assertEqual(code, 1)
        self.assertIn("1. ", out)

    def test_a_flood_is_grouped_and_the_detail_goes_to_a_file(self) -> None:
        self.init_audit()
        self.settle()
        self.flood()
        code, out = self.check_output()
        self.assertEqual(code, 1)
        detail = self.src / "check-detail.md"
        if "分 1 类" in out or "【" in out:
            self.assertIn("【", out, "报错聚合后应当按类分行")
        self.assertTrue(detail.is_file(), "全量明细应当落 check-detail.md：" + out)
    ...
    def test_the_detail_file_is_not_one_of_the_frozen_five(self) -> None:
        ...
        block = flow.split("STORY_SRC_FROZEN = (", 1)[1].split(")", 1)[0]
        self.assertNotIn("check-detail", block)


class TestBannedTermsSplitByArtifact(StoryBuildCase):
    """禁用词按产物分治：story 照旧拦，review 记一笔不拦。

    review 是决策文档——「可选的做法是什么」「为什么被否的那条不行」「不同意时
    改成什么」天然要写反事实与理由。实跑两处命中全属此类。它本来就有人工评审
    兜住，误伤面与兜底面重合。story 是叙事件，那些词出现在那里多半真是在讲交付机制。
    """

    def test_a_banned_term_in_the_story_still_fails(self) -> None:
        self.init_audit()
        self.settle()
        body = self.story()
        marker = "## 功能说明"
        self.assertIn(marker, body, "夹具变了，用例要跟着改")
        self.story_path.write_text(
            body.replace(marker, marker + "\n\n本方案采用灰度发布，先放开一部分用户。", 1),
            encoding="utf-8")
        self.assert_check_names("客户端语境禁用词")

    def test_a_banned_term_in_the_review_is_only_noted(self) -> None:
        self.init_audit()
        self.settle()
        ...
        review.write_text(existing + "\n不同意时改什么：文案回退为上一版措辞。\n",
                          encoding="utf-8")
        code, out = self.check_output()
        self.assertIn("这里只记一笔", out)
        self.assertEqual(code, 0, "review 的禁用词不该拦：" + out)


class TestDisambiguationReferenceIsExempt(StoryBuildCase):
    """术语表里为消歧拉进来的参照概念，不参与「story 里须找得到」判。

    参照物是 spec 的本职内容（Catalog 的易混项要求它出现在术语表），本需求
    并不做它，需求叙事里天然不出现。要求它出现，作者只能把不做的事写进 story
    ——那是为了过判据造事实。既有约定：解释列留「—」= 评审者不需要这个词。
    """
    ...
    def append_term_row(self, explain: str) -> None:
        """给 spec 造一张术语映射表，唯一一行是待判的那个词。

        spec.md 不在合同的 `sources` 里（那是 RR/SR/AR 三份材料），所以它不产生
        来源单元——加它只影响术语守恒这一条，基线的通过态不受影响。
        """
        spec = self.root / "doc" / "features" / FEATURE / "spec" / "spec.md"
        spec.parent.mkdir(parents=True, exist_ok=True)
        spec.write_text(
            "# 甲需求 spec\n\n## 0. 术语映射表\n\n"
            + self.HEADER + "\n|---|---|---|---|---|---|---|\n"
            + "| " + self.TERM + " | 别的模块 | 业务层 | 高 | — | [x] | " + explain + " |\n",
            encoding="utf-8")

    def test_a_term_with_a_real_explanation_must_appear(self) -> None:
        """对照组：解释列有内容 = 本需求的业务词，story 里缺了照旧点名。"""
        self.append_term_row("本需求里指某个具体业务动作")
        self.init_audit()
        self.settle()
        self.assert_check_names(self.TERM)

    def test_a_reference_term_marked_with_a_dash_is_not_required(self) -> None:
        """留「—」= 评审者不需要（被消费的基础能力那一类）。"""
        self.append_term_row("—")
        self.init_audit()
        self.settle()
        code, out = self.check_output()
        self.assertNotIn(self.TERM, out)

    def test_an_in_scope_reference_needs_an_explicit_mark(self) -> None:
        """权威模块也在 Scope 内的参照物，靠留空表达不了——spec 那边要求解释非空。

        这正是实跑里把作者夹住的形态：写了解释就得让它出现在叙事件，不写解释
        过不了 spec。所以标记是显式的一个值，加在解释列最前面。
        """
        self.append_term_row("（消歧参照）既有的另一条流程，与本需求那个不是一件事")
        self.init_audit()
        self.settle()
        code, out = self.check_output()
        self.assertNotIn(self.TERM, out, "标了消歧参照仍被要求出现：" + out)
...
class TestRetiredThings(unittest.TestCase):
    ...
    def test_the_manifest_version_covers_this_round(self) -> None:
        ...
        跳过 1.3.0：那个号已经有目标工程装过了。装过的目标看到同一个号会判「重适配」，
        而重适配**不执行机制行**——它恰好会跳过本轮改的那些文件。号只能往上走。
        """
