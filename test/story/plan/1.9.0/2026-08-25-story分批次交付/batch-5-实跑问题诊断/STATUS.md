...
| 项 | 值 |
|---|---|
| 状态 | **步骤 1～15 全部通过（15 经七轮返修，评审 2026-09-05 放行）。下一步：五跑** |
...
| 方案文件 | [05-实施方案总览.md](05-实施方案总览.md)；`steps/01`～`steps/15` |
...
| 13 adapt 带扩展依赖的 framework 改动 | **通过**：`951d9d61`；`framework-patch.yaml` 18 条按类别带（`extension_dependency` 无条件、`host_capability` 看目标物化了哪个 host），扩展升 1.4.0；adapt 不占机制预算（用户裁定） | — |
| 14 main → story 同步（framework 升 3.0.0） | **通过**：三步 `99b4d584` / `c39e50d4` / `07b7bb8a`，返修 `4008c443`（opencode 补丁按 `85e266f` 重生成、删三条陈旧 allowlist）。`framework/` 与 main 差异恰好 18 文件，与补丁清单逐一相等 | [reviews/14-main-to-story-sync.md](reviews/14-main-to-story-sync.md) |
| 15 四跑正向修正（F1–F9）+ 七轮返修 | **评审通过，可进五跑**（`reviews/15` §12）。四个原提交 `e71cd58d`/`d88f9b31`/`dda5c4d7`/`195acdb4`；返修 `6667809c` R1、`404320ce` R12–R14、`d3c9b064` R15+R4、`8eaa7f35` R17/R18/R20/R21、`f3fc4535` R22–R27+C3、`24d2dba5` R28/R29/R30+m1。647 绿、形态 70 条 FAIL 0 委派 8、预算 8732/8900 | [reviews/15-fourth-run-forward-fixes.md](reviews/15-fourth-run-forward-fixes.md) |
...
- 2026-09-05 评审者归因三跑 story 段五次停等：两次预期；第二轮材料停等是 SKILL 设计（用例脚本也按三次备好），其内容暴露导入无「只抽图」档；AR/design.md 覆盖确认是 ar_design_init 规则与推进契约矛盾；spec 入口授权是 SKILL 与 CLAUDE.md 红线 8 冲突下的不稳定。三处改扩展，一处（每轮都停）由用户定。
- 2026-09-05 用户裁定：材料关卡改为「补料后只在发现新缺口时再停」；停等消息要简洁明确、不口语化冗长。评审写入 steps/11「三跑观察后的修正」T1–T5（新缺口才停、只抽图导入档、删 design.md 覆盖确认、spec 授权转 framework 形态、停等消息 ≤12 行三段式），四跑前完成。
- 2026-09-05 用户裁定：需求方回话只回选项或一句短语；只抽图档由模型判断不问人。评审改 TEST.md §3.0（回话规约）与三个用例脚本（台词改立场短语），steps/11 T2 改写、加 T6。

## 步骤 12 · 三跑观察后的修正 T1–T5 · 已实施（四跑前）

按 `steps/11`「三跑观察后的修正」。T6（回话规约与用例脚本）评审已做。

### T1 · 补料之后不再问第二遍

人选「补充材料」就是对「材料够不够」的回答，补完再问一次问的是同一件事。
新一轮照样重新盘点（那是模型的活），**没有新缺口就直接进需求分析**；
盘出新缺口时把新选项写进侧车，`next` 随之回到材料关卡——**侧车在不在就是判据**
（`decide` 会消费掉它，盘上留着的只会是这一轮新写的）。

SKILL「停等点」节改写：这一处停几次由材料状态决定，不由轮次决定；`init_analysis.md` 同步。
三条夹具：无新缺口不停、有新缺口再停、第一轮照停。

### T2 · 补料只为补图时，正文不动

导入此前只有一条路——正文整体覆盖。于是「原稿放进去了，图在里面」的后果是定稿被草稿盖掉。
加归类档 `IMAGES`：抽图、登记，正文一个字节不动；**选哪一档由模型按材料判断，不再问人**
（人已经说过「补图」了）。归了这一档却没有图 → 报错，不静默丢正文。
`materials.py` 的「已并入」判定随之分两路：有正文落点的比正文，只抽图的比落地的图。
四条夹具：正文原样+图落地、归 RR 仍覆盖（反样本）、无图报错、材料版本仍然变。

### T3 · 删掉 `AR/design.md` 的覆盖确认

`ar_design_init.md` 第 4 条原文「已有非空 AR/design.md 时不静默覆盖（须经用户确认）」——
而需求系统拉下来的预填版每次都非空，这条等于每轮都停。S4 生成提取件本就是覆盖预填版，
旧版进 `.backup/`。全树无「不静默覆盖」。

### T4 · 进 spec 的授权接到 framework 认得的形态上

SKILL 原来写「S4 收口之后直接进 spec，不问」，理由是「用户发起 `/story` 时说的目标」。
但 framework 的推进策略（`user-confirmation-ux` §8.1）只认四种授权，「发起 /story」不在其中——
模型面对 CLAUDE.md 红线 8 只能再问一次，**它没错，是这条链没接上**。

接法：把 `/story <AR>` 的启动语义明写成 **batch 多阶段声明**（§8.1 第 2 条、§8.2），
声明范围 = 取材到 spec 闭环到归档；范围之外（plan 及其后）仍按默认策略停等。
`status` 在收口那一步**原样回显这句授权**，模型看到的不再是猜测而是一条写在规则里、
由脚本复述的声明。一条夹具守它。

### T5 · 停等消息三段，≤12 行

人在停等点只做一个动作——选一项；让他为此读三十行，是把成本从模型这边挪到他那边。
SKILL 加「停等消息怎么写」：一句现状、一句缺口或问题、选项各一行（推荐标出），
不放材料总表、不复述已知、不解释流程、不出现命令与判据名；选项文字写成他的话。
`scope_gate.md` 一二级的呈现要求按此改写——四列清单留在 `init-analysis.md`，
那是它存在的理由。

### 验收

- story 全量 **608 绿**（并行 21 s）；失效形态 70 条 FAIL 0、委派 15；`framework/` 零差异；
- 预算：`scripts_mjs` 1907、`scripts_py` 1371、`hooks_mjs` 2657、`prompts_md` 2026、
  `data` 683，总 **8644**（interim 8740）。`scripts_py` 峰值按实抬到 1400 并写明理由。

顺带修一处测试自身的脆弱：`ReviewTaskReachesTheVerifier` 拿仓内真实需求目录当输入，
而 CLI 起跑时装置会把 `doc/features` 整个迁走——改为自带最小需求目录。

### 下一步

起跑第四次 CLI。
- 2026-09-05 评审会话压缩前更新交接件 99：状态到四跑进行中；待评审 42bc5554（T1–T5）与 951d9d61（步骤 13）；四跑 story 段再现 5 次停等待归因。
- 2026-09-05 评审者观察四跑（61 分钟闭环，reviews/11 §14）：verifier 插件证据、三图全用、T2–T5 生效；story 段仍停 3 次——T1 判据绑在 decide 顺序上（先导后签被拒），且探针证明按设计顺序走到第二级关卡时 next 会拨回材料关卡；另三条机制缺口：任务包不给图片引用串（④/⑨ 四条、读源码 12 次）、附录 D/E 可派生内容让作者手写（29 条里 23 条）、verifier PASS 后重跑 harness 换 subject 致二次 verifier（10 分钟）。上下文 477K。42bc5554 判 T1 不通过。
- 2026-09-05 用户指出四跑三条：流程章画图为零（第二次，评审漏数）、章节形态不稳、哪些形态该固定。评审查明：story-template.md 写明了与金样一致的形态但全仓无人读它，skeleton 只写章名、任务包只给问题、check 无主叙事形态判据、S02/S08/S09/S11 类条目 responsibility=observed 无发现者；按金样列出固定/自由形态表与目标形态（形态进合同数据，skeleton/任务包/check 三处共用）。写入 reviews/11 §14.8。
- 2026-09-05 用户裁定：金样按具体需求写成，不能照抄；表不是越多越好。评审写 `steps/15-fourth-run-forward-fixes.md`：F1 形态真源（合同 form 定槽位只定必有列、skeleton 渲染、check ⑪ 守存在不守行数、模板退场、台账六条回 deterministic）、F2 任务包给可粘贴引用串与生成的附录 D/E 行、F3 材料关卡按轮次与本级侧车、F4 verifier 之后的路、F5 语言红线扫决策正文、F6 装置不改 summary 并数画出来的图；四提交，五跑判据与预算写明。待用户确认。
- 2026-09-05 独立评审步骤 14（main→story 同步到 3.0.0）：机制通过——18 处差异与声明一致、04 补丁在纯净 3.0.0 树上逐字节复现、@2 派生两侧一致、608 绿。一条返修：01 号 opencode 补丁与交接件停在 7401f22 基线，在 85e266f 上打不上且不含 @2；重生成。allowlist 三条陈旧可删。执行会话报告的产物分 90 评审认为偏高（80–85）。评审自己的步骤文件改号为 15（14 已被同步方案占用）。
- 2026-09-05 用户三条新诉求写入 steps/15：F7 story 从 spec 派生只增不减（术语 ⊇ §0、流程图复用 §5、附录 A/B/C 由 §9 与 Scope 生成，四跑对照：术语只重合 1/10、图 0、契约 18 行抄成 9 行）；F8 图的适用场景表与「章首一张完整图」规则；F9 固定标题三档与 spec §9 表头核。预算随之申请 target 8400。
- 2026-09-05 独立评审步骤 15（reviews/15）：全批规模——批次 4 起点 8819 → 退场 7787 → HEAD 8548，对起点净减 3%，退场后回长 10%（主体是生成与状态判断，判据块 15→17）；批次 4 根因逐条核：RC1/3/4/7/10 未回，RC2 一处边缘（specTerms 按 in_scope 选词），RC5 轻度重复，**RC8 回来一次**（F7 派生在 CRLF 真实 spec 上为空、夹具全 LF），**RC9 半回来**（target 一日三抬 8350→8400→8600）。返修 R1（行尾归一 + CRLF 夹具）；C1 请用户确认 8600 签字并冻结为本批最终值。
- 2026-09-05 评审补 reviews/15 §6 全扩展五问：脚本不全必须（自写 YAML 解析器 115、flow-check 核自己输出 218、下游五阶段义务门约 940 未经实跑且 coding 判法是标识串代理、生成后仍核作者的判据约 150）；脚本替模型判语义仅一处边缘；提示词约 1100 行、讲理由句占 16–25% 偏长；模型仍干三处机械活（copyedit 六行仪式、五份侧车 JSON 手写、20 多次命令编排）；主干正向，残留约 1500 行占 17%，建议收口前只删不加。
- 2026-09-05 用户裁定：把评审发现的全部问题写成返修意见交执行会话。reviews/15 §7 列 R1–R11：CRLF 派生、自写 YAML 解析器退场、flow-check 退场、下游义务门登记待实证并删标识串判法、生成后判据收窄、提示词只删不加至 ≤700 行、统稿仪式删除转 verifier、侧车 5→3 与 decide 参数、命令 ≤12、预算冻结 8600 待确认、夹具取真实产物。顺序 R1→R7→R8→R9→R6→R5→R3→R2→R4，完成后五跑。
- 2026-09-05 用户确认 8600 是自己签的，签这一笔正是发起全批膨胀质询的原因。8600 冻结为批次 5 最终 target，reviews/15 §7 清单只减不增。
- 2026-09-05 评审补 R12：步骤 15 的实现本身第二处实缺陷——骨架打底的术语行、流程图、附录五表在 `chapter` 整章替换时被作者章文件覆盖，F7 要消掉的重打被唯一的落盘路径要了回来；目标形态是生成区带标记、chapter 只替换作者区。R1、R12 是实现缺陷，R2–R11 是全扩展清理。
- 2026-09-05 用户质询「为什么把正常功能都要求清理了」。评审逐条复核后更正（reviews/15 §7.1）：撤回 R2（yaml 包扩展够不到，自写解析器是设计）、R3（flow-check 是防手改的兜底）、R8（内容走 argv 要经 shell）、R9（init/skeleton 顺序不可合）；R4 不删代码只登记下游未经实跑证明（对执行会话三选一：都不选）；R5/R6/R7 降为 advisory。仍成立：R1、R12（步骤 15 实缺陷）、R10、R11。
- 2026-09-05 用户纠正评审口径：字符限制是为发现「脚本代替模型决策」而设，重要的是方案与实现对不对，不是字符多少。评审改为逐段问「决策归谁」，不再以规模数字下结论；写入交接件规则 13 与记忆。
- 2026-09-05 执行会话按 §7 旧表准备开工 R1→R12→R7…R4，理解已过期。评审把 §7 表改为只列仍成立的 R1/R12/R10/R11，撤回与降级项移到 §7.1 并明写「不要做」；顺序 R1 → R12 → R4 登记一行。
- 2026-09-05 用户送来另一模型的批次 5 评审。评审逐条对代码核实后采纳四条进 reviews/15 §7：R12 细化为两种所有权（可重复投影归脚本、一次性种子归作者）；R13 固定槽位只写了注释没生成表头；R14 附录 D 依据、E 类别未从真源带出；R15 F4 文字让模型回填回执，而 3.0.0 回执由 harness 生成只读。R4 加描述改名「契约实体标识存在」。其规模数据与本评审一致，「只剩 R1/R12 过于乐观」的判断成立。清单现为 R1/R12/R13/R14/R15 + R10/R11。
- 2026-09-05 评审步骤 15 返修三提交：R1/R12/R13/R14/R15/R4 全部通过（拿真实产物走作者路径核实：生成区落盘不丢、术语种子不被打回、D 依据与 E 类别取真源、CRLF 夹具 blob 保真）。新发现 R16：投影把 spec §9「代码现状」列带进归档件，落盘后 ⑨ 仓内路径与 ⑩ 检索措辞必红且作者改不了；投影时去掉该列。预算 8600→8700「第三次签字」待用户确认。
- 2026-09-05 用户确认 8700 为自己所签。步骤 15 只剩 R16（投影去掉 spec「代码现状」列）；修完评审通过即进五跑。
- 2026-09-05 用户送来外部第二轮评审（针对 R12–R14 实现）。评审逐条核代码与 §8 实跑输出：六条全部成立、无误解需求；自认 §8 把 R12/R13 判早了（只验落盘不丢，没验作者填后与真源变后）。改判不通过，写 R17–R21 替换 R12/R13/R16；做完再评审，通过才进五跑。
- 2026-09-05 外部第三轮评审指出 R17–R20 两处根因未闭合（种子仍要模型搬进章文件；R19 落盘插占位是事后补结构）。评审核对后采纳：R17 改为 skeleton 生成作者直接编辑的章草稿（含槽位表头、种子、E 的类别与链接），chapter 消费草稿、缺槽位拒绝不插占位；R18 固定为 story 登记时 finalize：重投影→number→check→登记，number 不动；R20 只排除「代码现状」一列不设逐表白名单；R19 撤回。写入 reviews/15 §9.1。
- 2026-09-05 用户要求独立判断不默认认同。评审复核外部第三轮意见后两处不采纳并写明理由：chapter 不在落盘时拒绝缺槽位（形态判断只在 ⑪ 一处，避免第二处判断与拒改循环）；机器区投影不止登记前一次，落盘附录章时也从真源投一次（否则登记前 check 会因附录空节而红）。其余采纳。
- 2026-09-05 13:02 用户批准 reviews/15 §9.1 与 §9.2 为下一轮实现基线。外部第四轮三条执行契约核实后写死：chapter 落盘附录=作者区+当前真源机器区；投影唯一入口 story-build project，story 登记走 project→number→check；skeleton 只补缺失草稿不覆盖、登记后删草稿、E 材料变化由 ⑫c 报并指向重落盘。交执行会话实施。
- 2026-09-05 评审 8eaa7f35（R17/R18/R20/R21）：全部通过（真实产物走完作者路径：十份草稿、零种子骨架、project 单入口重投、草稿只补不覆盖、表间空行、代码现状列已去）；638 绿、8691/8700。新发现 R22 材料清单种子链接少一层 ../（⑫c 9 条，§8 时评审漏判）、R23 草稿的 HTML 指引与待写标记随落盘进归档件（⑩ 报 §0、已落盘章仍算待写）。修完再评审。
- 2026-09-05 14:00 外部第五轮意见七条核对全部成立（两条同 R22/R23）。新增 R24 机器区真源缺失响亮失败不写占位、R25 投影集合对账（合法为空投「不涉及」一行而非删区）、R26 只为待写章补草稿、R27 草稿删除须确认否则不登记、C3 预算文件与提交说明不一致（scripts_mjs 2088 > target 2070），先净减 18 行。修完再评审，不进五跑。
- 2026-09-05 14:45 评审 f3fc4535（R22–R27 + C3）：全部通过——真实产物上草稿零待写标记、E 链接对、指引被剥、真源缺项 project 退出 1、合同外旧区删、只补待写章草稿、删不掉不登记；阈值 2200/8900 用户授权且三处一致。步骤 15 通过，进五跑；两条 advisory（槽位单列锚、改动边界不投 §9.5 不涉及句）。
- 2026-09-05 15:10 外部第六轮意见四条核实全部成立，评审撤回 §11「通过、进五跑」：R28 整域不适用投影、R29 合同内空投影删旧区、R30 §9.5 不涉及句（advisory 升必修）、m1 预算文案。评审自省：两处 BLOCKER 是实跑没造出的场景，§9.5 降级是判错。
- 2026-09-05 15:30 评审 24d2dba5（R28/R29/R30 + m1）：真实产物上造整域不适用、§9.1 清空、§9.5 不涉及三个场景，全部符合；647 绿。步骤 15 通过，进五跑，判据 reviews/15 §12。
- 2026-09-05 用户要内网同步测试。评审核 adapt 现状：自适配 scan/check 通过、framework-patch 18 条不变、无引用已退场模板；要改一处——manifest 仍 1.4.0 而步骤 15 改了机制行为（草稿、project、侧车带 gate、IMAGES 档、合同 form、⑪/⑫b），不升版目标会判重适配不复制机制文件，须升 1.5.0 并改 test_story_build 断言。两条前提告知：内网 framework 须先同步到 3.0.0@85e266f 再 adapt（补丁是整文件，混版本会坏）；codex 无 verifier 发布器，spec 闭环到 verifier 那一步过不去，story 读者审查记不适用。
- 2026-09-05 评审写 artifacts/intranet-upgrade-1.3.0-to-1.5.0.md：内网（codex）从批次 4 升到 HEAD 的清单——前提两件（framework 先到 85e266f、manifest 升 1.5.0）；adapt 自动带 52 处机制变更 + 12 条作者入口补丁与 allowlist + manifest hooks + 入口段；人做的只有 .gitignore 与回话规约；codex 无 verifier 发布器，测试终点定在三份产物 + harness PASS + check 干净。
- 2026-09-05 用户裁定不写方案、评审直接改 adapt 并提交（内网 codex、扩展批次 4、framework 已 3.0.0）。按 1.4.0 那一步的思路只更新该更新的：manifest 升 1.5.0 并写明行为变化；adapt-scan 判态加机制指纹（版本相同而机制不同 → state=package_not_bumped，scan 报停、check ⓪ 不过）；.gitignore 要两行（adapt 工作目录、章草稿目录），scan 给出、check ⑦ 核；SKILL §1/§3/§6/§7 同步；夹具四条。
- 2026-09-05 15:50 五跑首次起跑卡在 verifier 之后：framework 3.0.0 的 request schema 已升 1.1，步骤 14·1.2 重打的 opencode 插件常量仍是 1.0，插件整份拒收（bedside invocation_request_unparseable），主模型 15:42 起在 framework 里查因。评审 reviews/14 §2 漏判（只比了 @2 字段没比 schema 常量）。修法：模板与物化件改 1.1、夹具加 harness↔插件 schema 对账、01 补丁重生成；工作区带旧插件，须重跑。story 段 3 次停等，T1 生效。
- 2026-09-05 16:10 评审五跑首次产物（reviews/15 §13）：作者在草稿上写、首次 check 仅 1 处、读源码 0、流程章有图、附录机器投影齐、story 段停 3 次；两处机制缺陷 R31（交接约定槽位无兄弟条件且单列锚表→多出一节）、R32（附录小节无 A.–E. 序号，模板退场后无人铺）；建议分产物 88 / 知识 90，性能待重跑。随 schema 1.1 一并修后重跑。
- 2026-09-05 评审写 steps/16-fifth-run-fixes.md（待确认）：S1 opencode 插件 request schema 1.0→1.1 + 三处常量对账夹具 + 01 补丁重生成；S2 交接约定加 siblings 条件、槽位表渲染推荐列而 ⑪ 只核锚；S3 number 给附录铺 A.–E.。一个提交，六跑判据加 verifier 恰好一次发布 canonical、无兄弟无交接约定、附录带字母、槽位表列数 ≥ 推荐列。预算 ≤ +20。
- 2026-09-05 用户指出 spec §10 不固定、story §3.3 一列表、§9 长文，要求逐节设计固定与自由。评审写 16-固定形态与自由形态设计.md（尺子：标题/形态/表头三层固定，行数措辞业务名永远自由；spec §9 表外无段落、§10 一条一行 ≤60 字且落点必填在 §9；story 十章逐节表：收紧 4.2/7.x/9.x 标题固定、9.2/9.3 成表、3.3 兄弟条件，放开 5.x/6.x/8.x 业务名）与 steps/17（合同全列+条件+labels/ordered 槽位、草稿渲染、⑪ 加不该有的节、post_check 三条、knowledge-use requirement 列表化），建议与 16 同轮做完再六跑。
- 2026-09-05 用户两条：固定形态不该大部分是表；16/17 合并。评审重写设计 §0/§2：固定形态按六种（叙述段·有序·无序·标签段·图·表）各定草稿渲染与机器核什么，9.2 改四项固定列表、9.3 改标签段，全篇固定表数量与四跑持平；steps/17 并入 steps/16 的 S2，两个提交（S1 单独；S2+S3）。
- 2026-09-05 16:30 外部第七轮评审核对成立：五跑 story 把 PRD 自带矛盾（未实名引导 vs AC-R1 看不到入口）传到验收、流程图节点悬空，作者与读者审查都没报——评审 §13 给 88 偏高，改 80–83 待六跑。steps/16 S2 五处修正：落点不强挂 §9（写类型）、字数不做门禁、4.2/4.4 真有才有且状态表不核、requirement 列表化补全消费者、verifier 判「同一产物不重审」并加真链路夹具；新增 S4 补作者统稿第七项与读者审查一致性一问，不加判据。设计文档同步。
- 2026-09-05 外部第八轮四条核实成立，局部修清：附录 D 依据完整保留全部条目不截首条；删掉残留的 ≤60/≤40 字与「落点必在 §9」旧条件（六跑判据、设计 §1/§3）；落点校验两路（§9 名字验真、非实体落点写实际影响对象由审查判，泛称不算）；S4 先核五跑证据（作者规则在没执行、verifier 无此问）再看六跑行为，「路径接回去」改「有明确后续或合法终态」。
- 2026-09-05 外部第九轮三条核实成立：落点类型改由作者显式声明（contract 验真 / impact 审查判），不按查找结果反推；S4 对 verifier 的归因收回——overlay 已列「前后矛盾」为 blocking 且五跑 ai-prompt.md 第 567 行已送达，属规则在、未发现，原因待证据，本步一问是补强；「无超过 5 句段落」「每节 2–6 行」降为建议。
- 2026-09-05 18:00 评审步骤 16 四提交（reviews/15 §14）：S1/S2a/S3/S4 通过，S2b 机制通过但 R33——post_check 的 strayProse 收到数组按逗号拼串，五跑真实 spec 的 §9 五节全误报「表外有段落」（夹具首行是表头所以没红）。追加 S5 spec 每张 mermaid 都进 story（%% 来源标记 + ⑫b 集合核）、S6 标签段改顺序提示不核、S7 图只引正文讲到的（⑫c 图连图/无承接两条机械事实）+ auto-topup 补料扩 10 张图含 1 张应不引用的竞品图。修完再评审进六跑。
- 2026-09-05 18:30 用户纠正 S5–S7：S5 改为正向——流程章结构从 spec §5 派生（spec 几张图 story 几个流程节，五跑少的是「管理与解约」这条流程），SR/PRD 的 mermaid 先进 spec 再进 story，来源标记机械核；S6 标签保留只换词（发生时机/系统怎么处理/之后去哪；回退动作/在途业务/为什么可控）；S7 装置图片只讲界面 10 张、删流程图 PNG、流程图放回 SR 的 mermaid。steps/16 与 reviews/15 §14.2 已改。
- 2026-09-05 用户定稿 S6 标签：时机 / 方案 / 走向；回退动作 / 兼容性说明 / 可靠性说明。S7 装置：docx 10 张界面图只有 5 张真要引，另 5 张是重复、旧版、竞品、无关页、兄弟需求页，判据是该引的 5 张各在讲它的节、不该引的一张不引且清单逐张写理由。steps/16 已改。
- 2026-09-05 用户再纠正 S5：图按内容归位进 story（内容在哪图在哪），图照搬文字不搬，不按图派生节；机械只核来源标记在不在，缺了报图的主题并指向「去看对应功能是漏图还是功能丢了」；任务包列 spec 的图与可粘贴围栏，归位由作者判。steps/16 S5 重写。
- 2026-09-05 19:00 外部第十轮七条核实全部成立：R34 落点验真收了表头且空 §9 跳过、R35 链路夹具写进 framework/ 且失败转 skip（红线 9）、S5 事实错（拿四跑夹具当五跑，五跑 §5.2 是触发流程；管理路径缺失是 spec 自身问题另计）、来源标记加唯一序、S7 与 S5 冲突改「在讲对应内容的位置」、兄弟需求页无依据改为运营台页/其它需求页并在 PRD 给依据、交付面两处「五跑」删并 M02 加「N 跑」。steps/16 与 reviews/15 §14.3 已改。
- 2026-09-05 用户纠正：步骤 16 已实施完，问题应按步骤 17 走。评审拆分：steps/16 只保留 S1–S4（已落地、评审不通过的记录），返修 R33/R34/R35/M02 与追加 S5/S6/S7 移入 steps/17-step16-rework-and-additions.md，三个提交，六跑判据与预算随之。
- 2026-09-05 上游发布 `0143e21` 后开批次 6（零改动适配）。批次 5 `steps/17`（未实施）迁为批次 6 `steps/02`，R35 随 opencode 插件退场删除；`steps/16` 指针改指批次 6。批次 6 步骤 1 方案评审不通过（`batch-6-零改动适配/reviews/01`）。批次 5 止于步骤 16（已实施、评审不通过），不是验收通过；后续在批次 6。
