# SR90004 交通卡紧急挂失恢复 — 系统设计

> 关联 RR90004。AR90004 承载钱包端页面、业务编排、上下文保存和卡云适配；账号能力与通用 UI 以既有出口被消费。
...
| 参与方 | 输入 | 输出 | 责任 | 失败边界 |
|---|---|---|---|---|
| WalletMain | 用户动作、脱敏卡片上下文、外部结果 | 页面状态、挂失请求、恢复动作 | 组织页面交互与业务路径，保存安全检查点 | 不绕过身份，不自行认定冻结成功 |
| AccountManager | 身份请求与回跳上下文 | 登录实名结果、当前账号范围 | 提供既有身份能力 | 失败时返回明确结果，不持有挂失申请 |
| 交通卡云适配层 | 脱敏映射后的卡片标识、账号范围、幂等键 | 资格、既有申请、创建结果、冻结状态 | 模拟本需求所需云侧协议并保持幂等 | 是资格和冻结结果的唯一事实源 |
| AR90005 补卡特性 | `freezeTicketId`、最终冻结状态 | 补卡订单、支付与制卡进度 | 承担费用确认、支付恢复、制卡跟踪和客服交接 | 不复制挂失或自行改变冻结状态 |
| 支付与通知能力 | 补卡订单、云侧状态事件 | 支付结果、状态提醒 | 由兄弟特性调用既有能力 | 支付中断不重复建单；通知失败不改变业务结果 |
| CommUI | 页面状态与可读文案 | 通用加载、提示、按钮和结果组件 | 提供现有视觉与交互基础 | 不承载挂失业务状态 |
...
云侧确认冻结后，本特性将 `freezeTicketId` 和最终冻结状态交给 AR90005。共享挂失上下文保留到补卡完成或取消；
补卡完成后清理，补卡取消后仍保留旧卡冻结结果。AR90004 未完成会阻塞 AR90005 联调和正式开放，AR90005 的
支付或通知失败不反向改变本特性的冻结结果。
...
WalletMain 负责本轮实现与端到端验收；账号与 CommUI 维护者只需确认既有出口的使用方式；卡云接口由本仓适配层
提供可控结果，用于覆盖正常、身份、冲突、响应丢失和失败路径。

交付时先联调资格、创建/复用和冻结结果，再由 AR90005 联调补卡支付与通知；开关默认关闭，观察错误分类和恢复结果后开放。
回退只关闭新入口并保留查询既有申请的能力，不删除处理中上下文，也不把通知失败解释为冻结失败。
