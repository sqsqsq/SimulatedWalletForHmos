...
sequenceDiagram
    ...
    W->>C: 查这个对象能给哪几档权限
    C-->>W: 该对象可用的权限档位
    Note over W: 权限区按返回的档位呈现；换对象则重新查
    O->>W: 选好权限、有效期，确认发出
    W->>C: 创建分享（带幂等标识）
    C->>M: 请求签发凭据
    alt 车厂云签发成功
...
### 4.1b `queryGranteePermission`

入参车辆标识与分享对象；返回该对象在这辆车上可用的权限档位。车主选定对象后调用，
权限区据它呈现；车主换一个对象要重新调，上一次的结果不再作数。

对象与车辆的关系由车云维护（家庭成员是车主在车机或 App 上加的，其他联系人默认只给最小权限）。
钱包不自己判断谁算家庭成员——那份关系不在端上，端上判会与车云对不齐。

对象不可分享时（例如对方没有开通车钥匙）返回原因，钱包据此提示车主换一个人，
此时权限区不呈现任何档位。
