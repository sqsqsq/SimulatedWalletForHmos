...
一次挂失从卡包进入，到用户看到真实冻结结果结束。各参与方的交互如下：
...
**什么时候发生**：查询资格时云侧返回需要登录或实名。

**页面怎么处理**：保留脱敏卡片上下文与检查点，转入既有身份流程。

**用户回来之后**：重新查询资格——不信任离开前的旧结果，身份流程期间账号与卡片状态都可能变化；核验通过就回到离开时的确认位置。若回跳发现账号变了，清空旧上下文，重新选择卡片。

### 5.3 卡片状态冲突的处理

**什么时候发生**：云侧卡片状态已变化，或发现同卡已有处理中的申请。

**页面怎么处理**：先刷新云侧状态，再结合已有申请判断，给出三种去向之一——

1. 云侧已有处理中的申请：**继续既有申请**，去确认它的结果；
2. 状态变化影响了此前的确认前提：**重新确认**后再提交；
3. 这张卡已无法在线处理：**转客服**。

判断依据永远是刷新后的云侧状态，不用端侧缓存。

### 5.4 网络中断与响应丢失的恢复

**什么时候发生**：查询、提交、结果确认的任何一步断网或响应丢失。

**页面怎么处理**：保留最近一次安全检查点。

**用户重试时**：先用同一幂等标识查询既有申请——申请已在云侧就复用它继续确认结果，不存在才重新走提交。无论断在哪一步，都不会产生第二份申请。

### 5.5 云侧调用时机

资格查询只发生在四个时机：打开页面、身份回跳、冲突恢复、用户重试。页面停留期间不轮询、没有后台任务；三条恢复路径处理完，都回到用户离开时的检查点，不从头再来。
...
先分清两类情况。**设计内的受限状态**不是故障——用户走到这里是流程设计的一部分，每个都有明确的出路：
...
出问题时关闭开关即回退，但回退**只隐藏新入口**，处理中的一切都保留：已提交的申请继续可查、未终结的上下文不删除、恢复能力不下线。原因是挂失不可半途蒸发——用户的卡可能已经在冻结流程里，回退若清掉这些，用户既看不到入口也查不到结果，等于把最需要保护的人晾在中间。补卡侧的通知失败也不解释成冻结失败。
...
- **产品需求**（RR90004，产品提供）——用户旅程、产品规则、原始验收意图（AC-R 系列）；
- **系统设计**（SR90004，系统设计提供）——两单拆分与分工、端到端时序、接口与存储约定、交付顺序；
- **开发需求分析**（AR90004，开发提供）——本单承载范围、与兄弟单的边界、验收意图承接；
- **界面原型说明**（产品另行提供，界面部分未走需求系统）——一页三段布局、页面状态走向、文案与可读性要求；图 1、图 2 出自此件。
