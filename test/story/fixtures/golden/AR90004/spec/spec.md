...
| 统计点 | 业务动作 | 实际适用结果 | 本端获知时机 | 已有或新增 | 代码现状 |
|---|---|---|---|---|---|
| 提交挂失 | 用户确认后提交或复用挂失申请 | 已受理 / 失败 | 创建或复用申请返回时 | 新增 | `WalletMain` 当前无挂失埋点 |
| 冻结结果 | 查询云侧冻结结果 | 已冻结 / 处理中 / 失败 | 冻结结果查询返回时 | 新增 | 同上 |
...
| 统计点 | 业务动作 | 实际适用结果 | 本端获知时机 | 已有或新增 | 代码现状 |
|---|---|---|---|---|---|
| 流程恢复 | 身份回跳、冲突恢复或重新进入后恢复流程 | 回到原检查点 / 从资格查询重来 | 恢复上下文读取完成时 | 新增 | 同上 |
...
|---|---|---|---|---|
| UX-01 | 基线 | 资格、恢复与结果三段的方向性布局参数用 start/end，文本对齐用 TextAlign.Start/End | 影响 · 挂失页面三段布局 | 模型 / 实机 |
| SEC-01 | 红线 | 页面、日志、上报与恢复记录只接收脱敏卡片标识与账号范围摘要 | §9.1 · loss_report_recovery_context | 模型 / 实机 |
| SEC-01 | 红线 | 恢复数据按账号范围隔离，不随备份恢复 | §9.1 · loss_report_recovery_context | 模型 / 实机 |
| DFX-01 | 基线 | 资格查询只在打开页面、身份回跳与冲突恢复时调用，页面停留期间不轮询 | §9.1 · queryLossEligibility | 模型 |
| DFX-01 | 基线 | 冻结结果只在提交返回后与用户主动刷新时查询 | §9.1 · queryLossEligibility | 模型 |
| DFX-02 | 基线 | 交付时给出新增文案资源的体积增量；不新增图片、三方 SDK 与独立安装包 | 影响 · WalletMain 资源清单与依赖清单 | 模型 |
| OBS-02 | 基线 | 资格、身份恢复、提交、结果与冲突各阶段经本地日志记录，并以同一流程关联标识串联 | §9.1 · queryFreezeResult | 模型 / 实机 |
| OBS-03 | 基线 | 统计冻结成功率与中断恢复成功率，覆盖提交挂失、冻结结果与流程恢复三个统计点 | §9.1 · 冻结结果 | 模型 / 实机 |
| OBS-05 | 基线 | 每个统计点每种结果在结果确定时上报一条，重复回调不产生第二条终态 | §9.1 · 提交挂失 | 模型 / 实机 |
| OBS-06 | 基线 | 三个统计点的上报编码先核仓内无既有登记，再在项目唯一登记位置登记 | §9.1 · 流程恢复 | 模型 / 实机 |
| RES-01 | 基线 | 界面图标使用系统符号，不新增图片 | 影响 · 挂失页面三段布局 | 模型 |
| RES-02 | 基线 | 新增文案登记在 WalletMain 资源中，中英文成对，页面只引用资源 key | 影响 · WalletMain 文案资源清单 | 模型 |
| COMPAT-01 | 基线 | 三个新增接口的出参缺字段时按失败或处理中收敛，不当作已冻结 | §9.1 · queryFreezeResult | 模型 |
| ENV-01 | 基线 | 清除恢复数据或缓存后重新进入，从无上下文状态重建挂失流程，不崩溃、不死循环 | §9.1 · loss_report_recovery_context | 模型 / 实机 |
| ENV-02 | 基线 | 重新进入、身份回跳与快速重复点击共享同一幂等状态，不重复创建挂失申请 | §9.1 · createOrReuseLossApplication | 模型 / 实机 |
| DLV-01 | 基线 | 新增页面文案形成翻译清单，跟踪中英文资源的回译与合入 | 影响 · WalletMain 文案资源清单 | 模型 / 人工 |

不命中的依据：
- OBS-07：材料没有运营采集诉求，AC-8 要求的是关键路径可观察，由运维统计承接
- COMPAT-02：三个接口的失败都收敛为页面可读失败状态，本需求不新增错误码，也不改动既有码的编号与含义
- DLV-02：挂失入口只在钱包内的交通卡详情页出现，不对其他应用开放；三个接口由钱包调用卡云，不对外提供
<!-- knowledge-use:end -->
| 编号 | 本需求的要求 | 落点契约名 |
|---|---|---|
...
| 适用单元 | 候选 | 命中信号或反证 |
|---|---|---|
| 挂失资格、提交、结果与重试流程 | decision-tree | 资格、身份、既有申请、已冻结 / 处理中 / 失败是互斥分支，每支各有后续步骤与失败处理 |
| 挂失页面三段与生命周期恢复 | page-interaction | 确认、身份回跳、冲突恢复、重试与客服由业务结果决定下一个交互 |
<!-- knowledge-use:end -->
| 适用单元 | 候选 | 命中信号或反证 |
|---|---|---|
