...
> [1.9.6.md](1.9.6.md)（上一版 [1.9.5.md](1.9.5.md)）;流程全景与演进方向见 [需求交付流程.md](需求交付流程.md);
...
知识与需求系统对接脚本由目标工程按真实代码与真实系统维护，`/story adapt` 不碰它们；升级时它列出要做的条目：知识与对接层从目标已适配到的版本（`adapted_for`）之后列起，在途单从目标升级前装的版本之后列起，逐版记录在 `doc/extensions/skills/story-adaptation/reference/upgrade-changes.md`。
...
| `/story update <编号>` | 已有产物之后,上游材料改版、评审意见回来或你有新决定:先问你要不要补料,再更新已有的规格、需求文档、评审件与方案 |
| `/story restore <AR>` | 回退上一次归档对系统正文的覆盖:用归档前留在需求目录 `.backups/cloud/` 的那一版恢复系统正文 |
| `/story help` | 输出流程说明 |
| `/story adapt <目标工程>` | 工程运维,不在开发顺序内:把这套流程装到／升级到另一个工程(见第七节) |

编号也可以不是 AR 单(问题单号、工单号):入口相同,材料由你提供(见 Q2),
只是没有归档环节——交付终点是仓内的规格与评审件。只有 AR 开头的编号会去需求系统取材、归档与回退。
...
flowchart TD
    ...
    S --> M["<b>你：会上没定的事拍板</b><br/>交了会议转写、且会上有没收敛或要推翻旧结论的事时才出现<br/>（没有会议、或会上都定了则跳过）"]
    M --> C("AI：需求分析，给出范围定法选项集")
    ...
    style M fill:#ffe8e8,stroke:#dc2626,stroke-dasharray: 5 5
...
**AI 只在这几处停下来问你**:补齐材料(第一轮必问)、会上没定的事(有会议且有要你定的话题才问)、
定范围(含怎么切、哪一份归本单)、交付门之后选先送审还是先做,加上「这个单号查不到」和归档、恢复这类
不可逆操作各自的确认。此外一律不问——门禁报错怎么修、下一步做什么,那是它的事。
...
- 在 **`review.md`** 上表态——AI 已把每个待决点整理成议题:决策点、依据,
  待确认的给可选做法与建议,已定的给结论与影响,末行写请哪个角色评审。

每条议题末尾同一个填写位:在「评审结论:」下勾**同意**、**需修改**或**暂缓**,
要改成什么、选别的做法、暂缓的原因写在「修改意见:」后面。待确认的议题,建议不等于已选,
同意就是同意建议;要选别的做法,勾需修改并写选哪一项。

**议题之外的发现**——缺的分支、该复用的组件、漏的埋点——写进末尾的
**「其他意见」**节,按 1. 2. 3. 编号,每条写清是什么、影响哪里。

表态完成后跑 `/story update`:AI 先问一次要不要补料,再按意见更新已有的规格、需求文档、评审件与方案,
改了什么、为什么、哪些没变都记在这一轮的 `update-notes.md` 里;更新前的现场留档,可整轮还原。
上游材料改版、你有新决定,也是同一条命令。
...
**Q4 已定的范围想调整?**
规格层面的调整写进评审意见后跑 `/story update`;范围本身要重定(update 不改范围),
...
**Q7 有需求澄清会的录音转写,怎么给?**
把转写 `.docx` 与其它材料一样放进 AI 报出的目录——它就是需求材料的一种,没有单独一步「交会议记录」。
AI 会先读会:只做字面纠偏并逐处留痕,再按话题整理出判断;判不准归属、会上没收敛、要推翻旧决定的,
会**在定范围之前单独停一次**一起摆给你,每个话题给推荐与选项;会上都定了、或这场会与本单无关,
这一次就不出现。你逐条表态后,AI 按原话与你的选择写出「当前会议结果」,后面的需求文档、
