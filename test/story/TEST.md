...
3. 在副本上重跑 verifier 子 agent（prompt 用 `phases/story-verify.md`）；
4. 数它裁出多少条「未讲清」，以及**点名的是不是你删的那几条**。

### 判据与处置

| 结果 | 含义 |
|---|---|
| ≥5 条「未讲清」且点名对上 | 模型层有区分力，这一轮的「讲清」可信 |
| 报了但点名对不上 | 它在猜。检查作业书是不是又掺进了整篇印象题 |
| **一条都没报** | **模型层无效**——这一轮所有「讲清」都不作数，回 `phases/story-verify.md` 看任务是否良定 |

不删事实时再跑一次作对照：结果应与现产物一致；不一致说明裁决本身不稳定。
