...
### 3.7 顺带观测：被测模型在 story 段内停下来问了没有

扩展给了它一条推进契约：用户启动 `/story` 即构成授权，story 流程段内（取材 → 关卡 →
定稿 → 归档）按契约一路走完，只有四种情形才停下问人——**材料缺口、范围变更、
同一处连续三次修不好、破坏性动作**。

所以实跑里出现下面这几类问话，都是**观测到的缺陷**，不是它谨慎：

| 它问 | 为什么算缺陷 |
|---|---|
| 「要不要进入 spec」 | 段内推进，授权已经给过了 |
| 「门禁报错了，要修吗」 | 报错自带修法，那是义务不是选择题 |
| 「下一步是跑门禁、还是叫审查、还是先人工评审」 | 那三件是同一条义务链，链内没有分叉 |

**怎么记**：这几类问话会以 `adaptive_reply_requests` 的形态出现在 poll 里（`question`
就是它的原话）。照 §3.0 以需求方身份回话推进，**同时把原话记下来**——收工后数一遍
「白名单之外的问话有几次」，那就是这条的读数。不要因为它问了就去改扩展文本，
一轮的读数说明不了是文本不到位还是这个宿主保守。

范围之外：spec 闭环之后要不要进 plan，**不在**这条观测里。那属于 framework 的阶段边界，
扩展不管，问了是对的。
