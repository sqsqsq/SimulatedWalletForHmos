...
- 模型给了编号选项：只回选项号或选项原文（「1」「整体承载」），不解释为什么；
...
**第 7 项的长期方向**：批次 5 的收口 target 是按现值定的（8350），那一轮不压缩规模。想让机制真正变小得另开需求，
起点是**约 5200 代码行**——它来自更早定下的 7500 全行方向，按注释占比折算到只数代码行的口径。
...
`baseline_coverage.py` 在步骤 9 随逐单元系统一起删除。它的口径是逐字命中，而金样对照过
一次的结论是——逐字命中会把一份理想文档判成大量丢失（`golden/README.md`）；它的枚举也
依赖已经退场的 `source-units.mjs`。语义守恒改由 §9.1 的审查任务与 §10 的三轴评分承担，
`fixtures/content-baseline/` 那三份基线留作历史样本，不再有工具读它。

## 9. 常驻行为回归：审稿者审不审得出「没讲」

> **定位（2026-09-03 用户裁定）**：本节与 §9.1 是**诊断工具**，不是任何步骤或批次的必跑项。正式评价只用正常需求跑真实
> Story，按 §10 三轴评分；审查者的区分力从「用户在真实产物上找到的问题它报没报」读出。只有当真实结果与审查者报告
> 明显对不上、需要定位是审查任务还是输入的问题时，才用本节的方法。

与 §8 并列，同样**只报数**（G8）。跑完一轮后，对**每个到达 spec 的 Case** 做一次。

story 的守恒分两层：脚本守确定性事实（章在不在、正文空不空、图片引用可不可解析、
材料清单对不对得上），独立审查守语义（讲了没有、讲清没有、是不是编的）。
**脚本层可以自证，审查层不能**——它说「没有阻断问题」，你没有别的办法知道它是不是在盖橡皮章。
...
4. 在副本上重跑 verifier 子 agent（它执行 spec overlay 里的 `story_reader_review`）；
5. 读它的 `blocking_findings`：点名的是不是你删的那 5 条、挖空那一章有没有被点出来。

### 判据与处置

| 结果 | 含义 |
|---|---|
| blocking 里点名了你删的那几条事实，挖空那一章也被点出来 | 审查有区分力，这一轮的结论可信 |
| 报了但点名对不上 | 它在猜。检查审查任务是不是又掺进了整篇印象题 |
| **一条都没报** | **审查层无效**——这一轮所有「无阻断」都不作数，回 spec overlay 的 `story_reader_review` 看任务是否良定 |
