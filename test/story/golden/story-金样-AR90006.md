...
| 交通卡云接口 | 查询策略、创建签约、更新签约、查询状态、解约 | 本地开发可先用模拟承载；真实业务开放前必须完成真实服务联调 |
...
签约时选的门限与面额、每日上限、连续失败停用这些规则，在签约、管理与自动充值里用同一组取值：
...
签约是用户主动完成的一次授权与建约；自动充值则是签约生效后的服务端过程。用户从交通卡页面进来，先过资格与身份，再选门限与面额并完成本人免密验证，卡云建约后进入管理页；之后余额触发的每一次充值都在云侧发生，用户在管理页看到结果，也可以修改、关闭，被停用后重新签约。
...
flowchart TD
    ...
    C -->|身份需补齐| I[进入既有实名流程]
    I -->|完成后回跳| B
    C -->|开关关闭或依赖不可用| X[暂停新签约；已签约用户照常管理]
...
- 身份需补齐：转入既有实名流程，完成后回到资格核对，不直接回到选择页；账号变了要重新开始（见 5.2）。
- 开关关闭或依赖不可用：暂停新签约，入口不隐藏；已签约用户照常进管理页查看记录、改档与关闭。
- 免密验证取消或失败：留在选择页，已选的门限与面额保留，不产生任何签约关系。
- 建约结果未确认：不显示成功也不显示失败，先向卡云确认；确认已有签约进管理页，确认未建立才继续办理。
- 云侧连续失败停用：由卡云判定，钱包只展示停用与原因；用户重新签约走同一条签约路径。
- 修改与关闭：都以云侧确认为准；关闭不撤销已经完成的充值订单，订单与签约关系分别处理。

### 5.2 签约时的跨方交接

签约这一步牵涉钱包、账号能力、支付部件和交通卡云四方。钱包先向卡云问可不可以签、有哪些档位；用户选好并点开启后，钱包请支付部件做一次本人免密授权，拿到授权结果再请卡云建约。本人授权和业务合约是先后两件事：授权通过不等于签约成功，只有卡云返回合约，钱包才把关系展示为已经建立。页面内的选择条件见功能说明。
...
- 查询失败或新签约不可开放：钱包向用户说明原因，不拉起授权。
- 身份需补齐且可续办：进入既有实名流程并保留续办上下文，回跳后重新核对可签约性，再决定恢复选择还是说明不可继续。
- 用户取消或验证失败：留在签约页，保留选择；没有任何签约关系产生。
- 授权成功：钱包携授权凭证与用户选择请求建约，结果有新合约、已有合约、未确认三种；已有合约直接进入管理，未确认按 5.1 的方式先确认再展示。
- 凭证边界：钱包不持久化支付授权凭证，也不在日志或事件里记录它；免密验证页面打开、验证完成和业务建约完成是三个不同节点，只有卡云返回业务合约后才展示为已建立。

### 5.3 管理、关闭与停用

已签约用户的修改与关闭都提交给卡云，以卡云确认为准；连续扣款失败导致的停用由卡云判定，钱包只展示原因。三条路径的结果都回到同一个管理入口，用户不必在多个页面找状态。
...
- 修改门限或面额：云侧确认后才显示新值；未确认时保留未成功说明，不假报新值已生效。
- 关闭：请求云侧解约，确认后关系关闭、后续不再触发；未确认同样不假报。
- 停用：由云侧在连续失败后判定，钱包展示停用状态与失败原因；重新签约回到签约资格与授权流程，建约确认后再回管理页。
...
| 功能开关关闭或配置不可用 | 暂停新签约，入口仍在 | 不因此解除既有签约；已签约用户照常管理 |
...
改门限与面额经签约更新提交，原签约关系不断，不走解约重签；只改门限不重新验证。调高面额是否要重新验证，待定：由支付与风控确认，确认前页面不承诺任一种。
...
| 编号 | 场景与可观察的通过条件 | 主责 |
|---|---|---|
| AC-R1 | 未实名用户点入口被引导去实名，完成实名前签不成约 | 钱包；账号与卡云配合 |
...
| 编号或检查项 | 可观察的通过条件 | 主责 |
|---|---|---|
| AC-R4 | 功能开关关闭后新签约暂停；已签约用户的记录、改档与关闭照常，入口不隐藏，自动充值继续由云侧执行 | 运营配置、钱包与卡云 |
...
签约转化率与签约成功率来自签约过程中策略查询、免密验证、建约三步各自的结果，管理操作成功率来自修改与关闭的结果；扣款、写卡与停用发生在卡云，由卡云统计。页面进入与点击已有自动采集，本需求只新增这些业务步骤的结果记录，明细在附录的埋点小节。上线后先看签约各步骤的失败分类，再看修改与关闭的成功率。
...
当前有依据的主动收回措施是运营关闭功能开关。它只管新签约，不是整个自动充值业务的总停止开关：已签约用户的记录、改档与关闭照常。
...
| `updateAutoTopupContract` | `contractNo`、门限、面额 → 更新结果 | 改门限与面额时调用，原签约关系不断；只改门限不重新验证 |
| `cancelAutoTopupContract` | `contractNo` → 解约结果 | 解约确认后停止后续触发；不撤销已完成订单，进行中订单按云侧业务处理 |

调高面额是否重新验证待支付与风控确认；授权后建约失败时端侧怎么处理，待卡云与支付对齐协议号有效期后确认。支付验证的入参与协议号产生时点在联调契约中明确。错误码上游未给出，不以“无新增”替代失败语义。
...
这次改动落在钱包主模块与路由接入，登录实名、界面组件与公共能力只复用；业务责任的划分见范围。
...
激活的每条规约落到哪个实现或检查位置，逐条登记在这里；列出编号不等于落实。
...
- **产品需求**——需求系统 RR90006《交通卡自动充值》（产品，归档版）：业务目标、档位、限额与验收意图 AC-R1–R9。原文：[RR/prd.md](../RR/prd.md)；
- **系统设计**——需求系统 SR90006（系统设计，归档版）：参与方、签约与管理流程、接口、失败恢复及交付依赖。原文：[SR/design.md](../SR/design.md)；
- **开发需求分析**——需求系统 AR90006（开发）：钱包端责任范围；需求方确认签约与管理能力整体承载。原文：[AR/design.md](design.md)；
- **产品原稿**——《交通卡自动充值》v0.4（钱包产品组另行提供）：界面与档位，正文图片出自此件。原文：[inbox/交通卡自动充值.docx](../inbox/交通卡自动充值.docx)；
- **澄清会记录**——《交通卡澄清会记录-0903》（需求方提供）：签约更新接口、AC-R1 与开关口径的修正，以及仍待定的两件事。原文：[inbox/交通卡澄清会记录-0903.docx](../inbox/交通卡澄清会记录-0903.docx)。
