...
本表供联调核对请求、结果和调用条件。保留上游已声明的接口名称，不把未给出的协议细节写成已经存在的实现。
...
本节给出精确名称及其使用边界；两个本地数据对象的业务区别见6.6。
...
| 功能配置 | 运营下发的新签约入口开关与档位 | 不可用按关闭处理；业务初始开放状态由运营配置确定，不在钱包私设默认开启 |
| 事件 | `auto_topup_signup_enter`、`auto_topup_contract_created`、`auto_topup_verify_cancel/fail`、`auto_topup_modify/close`、`auto_topup_disabled_view` | 名称沿用本轮工程设计；只带允许的最小字段，上报失败与业务结果分离 |

斜杠连接的事件名表示一组相关事件，具体名称以实现契约登记为准，不表示把两个不同结果作为同一次成功统计。重新尝试属于过程，不能替代成功、失败或取消的结果记录。
