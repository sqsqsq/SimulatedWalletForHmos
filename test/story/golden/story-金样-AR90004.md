...
完整的需求分成两段：先冻住旧卡，再办新卡。前一段是本特性，后一段由兄弟特性「补卡」承载。
...
用户在卡包里发起紧急挂失，页面先向卡云查资格与既有申请：资格可用就让用户确认风险后提交；身份没完成先去补身份再回来重查；卡片状态有冲突或已有申请在处理，按卡云给的依据决定继续、重新确认还是找客服。成功只认卡云返回的已冻结。资格查询只发生在四个时机：打开页面、身份回跳、冲突恢复、用户重试——页面停留期间不轮询，也没有后台任务。
...
- 资格可用：用户读完冻结影响并确认后才能提交；提交是创建申请或复用已存在的申请，然后查询一次真实冻结结果（5.1）。
- 身份未完成：保留脱敏的卡片上下文与检查点，转入既有身份流程，回跳后重新查询资格；账号变了就清空旧上下文重新选卡（5.2）。
- 卡片状态冲突或已有处理中的申请：刷新状态与既有申请，按卡云给的依据决定继续、重新确认或转客服（5.3）。
- 结果只认卡云：已冻结才展示成功，处理中展示可恢复的状态，失败给原因与下一步；提交或查询途中网络中断按 5.4 恢复。
...
上线后看两个比例：提交挂失后云侧确认冻结的比例，以及中断后回到原检查点的比例；统计点在附录的埋点小节。
...
3. **灰度观察**：开关保持关闭，观察冻结成功率与中断恢复成功率里失败与处理中的分类，确认失败不会伪装成成功、恢复路径真实可用；
4. **开放**：观察达标后打开入口。
...
<!-- story-build:begin 技术约定·接口 · 由spec §9.1.1生成，改它请改真源 · sha256:4a950c5d60e7e3dc -->
| 接口 | 性质 | 输入 → 输出 | 错误码 |
|---|---|---|---|
| `queryLossEligibility` | 新增 WalletMain 卡云适配接口 | 脱敏映射后的 `cardId, accountScope` → `eligible, identityRequired, cardState, existingApplication, reason` | 资格查询失败由页面收敛为可读失败状态 |
| `createOrReuseLossApplication` | 新增 WalletMain 卡云适配接口 | `cardId, accountScope, userConfirmation, idempotencyKey` → `applicationId, submitted` | 创建或复用失败由页面收敛为可读失败状态 |
| `queryFreezeResult` | 新增 WalletMain 卡云适配接口 | `applicationId` → `frozen, processing, failed, reason` | 非 `frozen` 不得映射为成功 |
<!-- story-build:end -->

有效申请由创建或复用接口返回原申请号，重试不会建出第二份。冻结成功后本特性向补卡单向交出 `TrafficCardFreezeHandoff`（账号、卡片、冻结凭证与最终冻结状态），不带补卡字段；非冻结终态不产生交接。

#### 10.1.2 数据

<!-- story-build:begin 技术约定·数据 · 由spec §9.1.2生成，改它请改真源 · sha256:4912900ede27490a -->
| 键名/表名 | 介质 | 值结构 | 有效期 | 用途 |
|---|---|---|---|---|
| `loss_report_recovery_context` | relationalStore | `maskedCardId, accountScopeHash, checkpoint, idempotencyKey, applicationIdDigest, updatedAt` | 最终成功、明确失败、账号变化或超过有效期时清理；补卡协作期按上游共享约定保留 | 页面返回、身份恢复和重试查询时恢复同一流程；按账号隔离且不随备份恢复 |
<!-- story-build:end -->

#### 10.1.3 配置

<!-- story-build:begin 技术约定·配置 · 由spec §9.1.3生成，改它请改真源 · sha256:1dc11c76c064fddc -->
| 配置项 | 默认值 | 行为 |
|---|---|---|
...
<!-- story-build:begin 技术约定·埋点 · 由spec §9.1.4生成，改它请改真源 · sha256:422f57c925d9eac1 -->
运维统计紧急挂失的冻结结果与中断恢复，用来定位用户卡在哪一段（AC-8 要求关键路径可观察）；上报只带匿名入口、阶段与结果分类，上报失败不改变挂失结果。

##### 10.1.4.1 挂失冻结成功率

衡量提交挂失申请后云侧确认冻结的比例：分子是冻结结果为已冻结的次数，分母是提交挂失申请的次数；处理中与失败是未冻结的两种去向，分别计数供定位。

| 统计点 | 业务动作 | 实际适用结果 | 本端获知时机 | 已有或新增 |
|---|---|---|---|---|
| 提交挂失 | 用户确认后提交或复用挂失申请 | 已受理 / 失败 | 创建或复用申请返回时 | 新增 |
| 冻结结果 | 查询云侧冻结结果 | 已冻结 / 处理中 / 失败 | 冻结结果查询返回时 | 新增 |

##### 10.1.4.2 中断恢复成功率

衡量挂失流程中断后回到原检查点的比例：分子是恢复后回到用户离开时那一步的次数，分母是发生恢复的次数。

| 统计点 | 业务动作 | 实际适用结果 | 本端获知时机 | 已有或新增 |
|---|---|---|---|---|
| 流程恢复 | 身份回跳、冲突恢复或重新进入后恢复流程 | 回到原检查点 / 从资格查询重来 | 恢复上下文读取完成时 | 新增 |
<!-- story-build:end -->
...
<!-- story-build:begin 规约判定 · 由spec/knowledge-use.yaml生成，改它请改真源 · sha256:d45ad8bad7687c97 -->
| 规约域 | 编号 | 判定 | 依据 |
|---|---|---|---|
| 兼容性检查表 | COMPAT-01 | 命中 | 三个新增接口的出参缺字段时按失败或处理中收敛，不当作已冻结 |
| 兼容性检查表 | COMPAT-02 | 不命中 | 三个接口的失败都收敛为页面可读失败状态，本需求不新增错误码，也不改动既有码的编号与含义 |
| 资料交付 | DLV-01 | 命中 | 新增页面文案形成翻译清单，跟踪中英文资源的回译与合入 |
| 资料交付 | DLV-02 | 不命中 | 挂失入口只在钱包内的交通卡详情页出现，不对其他应用开放；三个接口由钱包调用卡云，不对外提供 |
| DFX 基线（性能/功耗/ROM/RAM） | DFX-01 | 命中 | 资格查询只在打开页面、身份回跳与冲突恢复时调用，页面停留期间不轮询 |
|  | DFX-01 |  | 冻结结果只在提交返回后与用户主动刷新时查询 |
| DFX 基线（性能/功耗/ROM/RAM） | DFX-02 | 命中 | 交付时给出新增文案资源的体积增量；不新增图片、三方 SDK 与独立安装包 |
| 环境异常标准场景库 | ENV-01 | 命中 | 清除恢复数据或缓存后重新进入，从无上下文状态重建挂失流程，不崩溃、不死循环 |
| 环境异常标准场景库 | ENV-02 | 命中 | 重新进入、身份回跳与快速重复点击共享同一幂等状态，不重复创建挂失申请 |
| 可观测性 | OBS-02 | 命中 | 资格、身份恢复、提交、结果与冲突各阶段经本地日志记录，并以同一流程关联标识串联 |
| 可观测性 | OBS-03 | 命中 | 统计冻结成功率与中断恢复成功率，覆盖提交挂失、冻结结果与流程恢复三个统计点 |
| 可观测性 | OBS-05 | 命中 | 每个统计点每种结果在结果确定时上报一条，重复回调不产生第二条终态 |
| 可观测性 | OBS-06 | 命中 | 三个统计点的上报编码先核仓内无既有登记，再在项目唯一登记位置登记 |
| 可观测性 | OBS-07 | 不命中 | 材料没有运营采集诉求，AC-8 要求的是关键路径可观察，由运维统计承接 |
| 资源使用 | RES-01 | 命中 | 界面图标使用系统符号，不新增图片 |
| 资源使用 | RES-02 | 命中 | 新增文案登记在 WalletMain 资源中，中英文成对，页面只引用资源 key |
| 安全隐私 | SEC-01 | 命中 | 页面、日志、上报与恢复记录只接收脱敏卡片标识与账号范围摘要 |
|  | SEC-01 |  | 恢复数据按账号范围隔离，不随备份恢复 |
| UX 一致性 | UX-01 | 命中 | 资格、恢复与结果三段的方向性布局参数用 start/end，文本对齐用 TextAlign.Start/End |
<!-- story-build:end -->
