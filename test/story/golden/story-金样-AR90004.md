...
挂失的进入、结果、恢复三个阶段各记一次结果，用来看失败集中在哪个阶段；明细在附录的埋点小节。
...
| 条目 | 约定 |
|---|---|
| `loss_report_recovery_context` | 保存 `maskedCardId`、`accountScopeHash`、`checkpoint`、`idempotencyKey`、`applicationIdDigest`、`updatedAt`；按账号隔离，不随备份恢复；最终成功、明确失败、账号变化或过期时清理，补卡协作期按上游共享约定保留 |

#### 10.1.3 配置

| 条目 | 约定 |
|---|---|
| `traffic_card_emergency_loss_enabled` | 默认 `false`；关闭或读取失败时隐藏入口，旧版本读不到按关闭处理 |

#### 10.1.4 埋点

事件只带匿名入口、阶段与结果分类；上报失败不改变挂失结果。

##### 10.1.4.1 挂失阶段结果分布（紧急挂失流程）

衡量挂失从进入到冻结结果、再到中断恢复的各阶段结果，用于定位失败阶段（验收 AC-8 要求关键路径可观察）。

| 统计点 | 业务动作 | 实际适用结果 | 本端何时得知 | 已有或新增 |
|---|---|---|---|---|
| 挂失进入（`loss_flow_start`） | 用户进入挂失流程 | 进入 | 进入流程时 | 新增 |
| 挂失结果（`loss_flow_result`） | 取得冻结结果 | 已冻结 / 处理中 / 失败 | 冻结结果查询返回时 | 新增 |
| 挂失恢复（`loss_flow_recovery`） | 中断后恢复流程 | 恢复 | 发生恢复时 | 新增 |

- 每个阶段的终态只记一次，重复回调不产生重复记录。
...
本需求激活的工程规约逐条判定结果，依据尽量指向可验的验收编号。
...
| 可观测性 | OBS-03 | 命中 | 每业务阶段终态只记录一次，重复回调不产生重复记录（附录 10.1 埋点约定）。 |
