...
|---|---|---|---|---|---|
| 卡号校验 | 通过 | BankCardOpenFlow.submitInfo | 内码 1001010100，STEP_SUCCESS | 「卡号校验通过」，拼卡类型（取自卡 BIN 识别结果） | 每次提交一次尝试；合法卡号一条 |
...
| 信息校验整体 | 失败 | BankCardOpenFlow.submitInfo | 内码 1001010001，STEP_ERROR_BY_ERROR | 「信息校验失败」，拼失败的子点（卡号校验 / 身份校验，取自各子点的结算结果） | 更正卡号后再提交记两次尝试 |
| 信息校验整体 | 用户取消 | BankCardOpenFlow.submitInfo | 内码 1001010010，STEP_ERROR_BY_USER | 「信息校验阶段用户取消」，拼取消时所在子点（取自开卡业务控制者记录的当前子点） | 用户终止一条 |
...
| 验证码发送 | 服务失败 | SmsVerifyController.send | 内码 1001020102，STEP_ERROR_BY_ERROR，外码取短信接口返回码 | 「验证码发送服务失败」，拼异常类型（超时 / 网络 / 服务端错误，取自请求异常） | 服务失败一条 |
| 验证码校验 | 通过 | SmsVerifyController.verify | 内码 1001020200，STEP_SUCCESS | 「验证码校验通过」，拼这是第几次输入（取自本次开卡的输入计数） | 每次提交验证码一次尝试；先错后对记两条 |
| 验证码校验 | 错误 | SmsVerifyController.verify | 内码 1001020201，STEP_ERROR_BY_ERROR，外码取校验接口返回码 | 「验证码错误」，拼这是第几次输入（取自本次开卡的输入计数） | 错误一条，重输是新尝试 |
| 验证码校验 | 过期 | SmsVerifyController.verify | 内码 1001020202，STEP_ERROR_BY_ERROR，外码取校验接口返回码 | 「验证码过期」，拼距发送的秒数（取自发送时刻） | 过期一条 |
| 验证码校验 | 服务失败 | SmsVerifyController.verify | 内码 1001020203，STEP_ERROR_BY_ERROR，外码取校验接口返回码 | 「验证码校验服务失败」，拼异常类型（超时 / 网络 / 服务端错误，取自请求异常） | 服务失败一条 |
| 短信验证整体 | 成功 | SmsVerifyController.verify | 内码 1001020000，STEP_SUCCESS | 「短信验证完成」，拼共输入几次（取自本次开卡的输入计数） | 可恢复的输错不结束整体；先错后对只记一条成功 |
| 短信验证整体 | 失败 | SmsVerifyController.verify | 内码 1001020001，STEP_ERROR_BY_ERROR | 「短信验证失败」，拼无法继续的原因（输错次数用尽 / 过期未重发 / 服务失败，取自各子点的结算结果） | 无法继续时一条 |
| 短信验证整体 | 用户取消 | SmsVerifyController.verify | 内码 1001020010，STEP_ERROR_BY_USER | 「短信验证阶段用户取消」，拼已发送次数（取自发送计数） | 用户退出一条 |
...
| 身份比对 | 服务异常 | FaceVerifyController.compare | 内码 1001030202，STEP_ERROR_BY_ERROR，外码取比对接口返回码 | 「身份比对服务异常」，拼异常类型（超时 / 网络 / 服务端错误，取自请求异常） | 服务异常一条 |
| 人脸核验整体 | 成功 | FaceVerifyController.compare | 内码 1001030000，STEP_SUCCESS | 「人脸核验完成」 | 检测与比对都有结果时一条 |
| 人脸核验整体 | 核验失败 | FaceVerifyController.compare | 内码 1001030001，STEP_ERROR_BY_ERROR | 「人脸核验失败」，拼失败的子点（活体检测 / 身份比对，取自各子点的结算结果） | 核验失败一条 |
| 人脸核验整体 | 拒绝授权 | FaceVerifyController.compare | 内码 1001030002，STEP_ERROR_BY_ERROR | 「人脸核验拒绝授权」，拼被拒的权限（取自权限回调） | 拒绝授权不编子点结果；一条 |
| 人脸核验整体 | 用户取消 | FaceVerifyController.compare | 内码 1001030010，STEP_ERROR_BY_USER | 「人脸核验阶段用户取消」，拼取消时所在子点（取自开卡业务控制者记录的当前子点） | 用户中止一条 |
...
| 开卡申请 | 服务异常 | CardOpenService.apply | 内码 1001040102，STEP_ERROR_BY_ERROR，外码取开卡接口错误码 | 「开卡申请服务异常」，拼异常类型（超时 / 网络 / 服务端错误，取自请求异常） | 服务异常一条 |
| 本地写入与激活确认 | 成功 | CardOpenService.writeCard | 内码 1001040200，STEP_SUCCESS | 「写卡与激活确认成功」，拼是否经过待确认查询（取自写卡流程状态） | 待确认查明后只记一次；超时待确认、查明成功一条 |
...
| 写卡整体 | 失败 | CardOpenService.writeCard | 内码 1001040001，STEP_ERROR_BY_ERROR | 「写卡失败」，拼权威结果与本地写入各自的状态（取自激活确认响应与写卡组件回调） | 权威结果与本地写入都确定时一条 |
| 结果数据获取 | 成功 | OpenResultLoader.load | 内码 1001050100，STEP_SUCCESS | 「开卡结果数据获取成功」 | 页面曝光不记 |
| 结果数据获取 | 失败 | OpenResultLoader.load | 内码 1001050101，STEP_ERROR_BY_ERROR，外码取结果接口返回码 | 「开卡结果数据获取失败」，拼异常类型（超时 / 网络 / 服务端错误，取自请求异常） | 不改写开卡结果；获取失败而开卡成功一条 |
| 开卡全流程 | 成功 | BankCardOpenFlow.finish | 内码 1001000000，SUCCESS | 「开卡完成」 | 一次开卡只记一次；正常开卡一条 |
| 开卡全流程 | 失败 | BankCardOpenFlow.finish | 内码 1001000001，STEP_ERROR_BY_ERROR | 「开卡失败」，拼失败所在步骤（信息校验 / 短信验证 / 人脸核验 / 写卡，取自各步骤的结算结果） | 卡号失败一条 |
| 开卡全流程 | 用户取消 | BankCardOpenFlow.finish | 内码 1001000010，STEP_ERROR_BY_USER | 「开卡用户取消」，拼取消时所在步骤（取自开卡业务控制者记录的当前步骤） | 取消一条 |
| 查询参数校验 | 通过 | OpenStatusQuery.query | 内码 1002010100，STEP_SUCCESS | 「查询参数校验通过」 | 每次查询一次尝试 |
| 查询参数校验 | 无效 | OpenStatusQuery.query | 内码 1002010101，STEP_ERROR_BY_ERROR，本地校验不带外码 | 「查询参数无效」，拼无效的参数项名（取自本地参数校验结果） | 参数无效时不发请求 |
| 开卡状态查询 | 成功 | OpenStatusQuery.fetchStatus | 内码 1002020100，STEP_SUCCESS | 「开卡状态查询成功」，拼查到的开卡状态（取自状态查询响应） | 接口重试按实际尝试分别记；查到「开卡失败」仍记成功 |
| 开卡状态查询 | 业务失败 | OpenStatusQuery.fetchStatus | 内码 1002020101，STEP_ERROR_BY_ERROR，外码取查询接口返回码 | 「开卡状态查询业务失败」，拼查询接口返回的原因说明 | 业务失败一条 |
| 开卡状态查询 | 服务失败 | OpenStatusQuery.fetchStatus | 内码 1002020102，STEP_ERROR_BY_ERROR，外码取查询接口返回码 | 「开卡状态查询服务失败」，拼异常类型（超时 / 网络 / 服务端错误，取自请求异常） | 服务失败一条 |
| 查询整体 | 成功 | OpenStatusQuery.query | 内码 1002000000，SUCCESS | 「开卡结果查询完成」，拼查到的开卡状态（取自状态查询响应） | 不触发开卡流程上报 |
| 查询整体 | 失败 | OpenStatusQuery.query | 内码 1002000001，STEP_ERROR_BY_ERROR | 「开卡结果查询失败」，拼失败的子点（参数校验 / 状态查询，取自各子点的结算结果） | 查询失败不改开卡结果 |
