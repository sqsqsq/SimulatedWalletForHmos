...
- 2026-09-05 用户裁定：金样按具体需求写成，不能照抄；表不是越多越好。评审写 `steps/14-fourth-run-forward-fixes.md`：F1 形态真源（合同 form 定槽位只定必有列、skeleton 渲染、check ⑪ 守存在不守行数、模板退场、台账六条回 deterministic）、F2 任务包给可粘贴引用串与生成的附录 D/E 行、F3 材料关卡按轮次与本级侧车、F4 verifier 之后的路、F5 语言红线扫决策正文、F6 装置不改 summary 并数画出来的图；四提交，五跑判据与预算写明。待用户确认。
