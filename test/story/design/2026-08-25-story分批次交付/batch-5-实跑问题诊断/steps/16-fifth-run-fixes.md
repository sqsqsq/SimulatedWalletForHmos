# 步骤 16 · 五跑后的修正：schema、固定形态落地、附录序号，修完重跑

**状态**：方案，待用户确认后实施。证据：`reviews/14` §5（schema）、`reviews/15` §13（产物）。
五跑首次起跑（`story-suite-20260905-145704`）story 段 3 次停等、作者在草稿上写、首次 check 1 处、读源码 0 次、流程章有图、附录机器投影齐——步骤 15 的方向已经对了。本步做三件事：S1 schema 缺陷；S2 按 `16-固定形态与自由形态设计.md` 把固定形态整套落地（含 3.3 兄弟条件与 §9 长文的根因）；S3 附录序号。一轮做完再六跑。
...
三个提交：S1 单独（常量 + 两条夹具 + 补丁重生成，先落）；S2 + S3 一个提交（合同、草稿、number、post_check、knowledge-use 消费者）；S4 一个提交（两份文本 + 行数常量）。提交后：全量并行绿、失效形态 70 条 FAIL 0、本仓自适配 `--check` 通过、`framework/` 与 main 仍恰好 18 文件差异、01 补丁在纯净 3.0.0 树上 apply 后与 story 逐字节一致。
...
| spec | §9 表外零段落；§10 每条要求独立可懂、必要条件齐、命中行落点齐 |
