...
1. 读取最终 run 的 `<features_dir>/<feature>/testing/reports/device-test-timing.json`（不得复用首轮真编/旧轮 timing）。填充测试概览「真机流水线耗时」表，使用最终 `build_reused`/`install_reused` 与 `hapBuiltAt`；打包/装机说明列显式写 `reused=true|false`；执行结果表增加耗时列，并为每个 case 回填最终 `duration_ms`，报告格式固定为精确整数毫秒 `Nms`（如 `1234ms`；历史 `1,234ms` 可读）。已进入 trace/timing 的 skip/block case 写 `0ms`，仅未进入 trace/timing 的 explicit skip 写 `—`。
2. 解析最终 authoritative `trace.json`：`cases[]` 每条含 `id`（与派生表用例编号对齐）、`status`（通过/失败/阻塞/跳过）、`notes`（可选）；skip 必须保留并计入对应分母，不能从统计中删除。native trace 还须以 `trace.artifacts.plan` 和既有 run/identity receipt 绑定实际 derived plan/trace；StepResult count/index/kind 不一致即 FAIL。
3. 构建行集：派生表中出现的 TC 以 `cases[]` 为准写状态与备注（无 case 记录但 run 整体失败→标阻塞或失败并注原因）；仅在顶层 test-plan.md、未进派生表的 TC → 标跳过，备注示例「缺少稳定 selector，需补 plan.md/contracts.yaml」。这类 explicit skip/未执行项即使是 P1/P2 也保持 testing FAIL，不能因 P0 已通过而完成 testing；只有机器证明的 capability 缺失可 defer。
4. **不要**与 Hylyre 状态枚举混用其它字样（门禁与 receipt 校验依赖一致词表）。

**报告重算**：当 run 已产出 trace、agent 已基于最终 trace/timing 写好顶层 `test-report.md` 后，可执行 `--report-reconcile-only --phase testing --feature <feature>`。该模式只读 trace/plan/report/timing/build-install-run meta，先对账同一最终 run 的 HAP 路径/内容指纹、meta 时间顺序、trace feature、精确 trace/timing case 集合、pipeline reused/阶段耗时和报告逐 case 耗时，再完整重跑 report/static checks 与既有 summary/quality axes writer；不调用 hvigor、hdc、Hylyre、设备、视觉采集或 executable lifecycle hook，也不改 trace 字节。
...
- **P0 用例 skip 证据边界（c7e4a2d9 / testing-stepresult-evidence-consumption）**：
  explicit_skip/未执行的 P0 → `p0_coverage_integrity` BLOCKER（fail-closed 不变）。没有
  `CaseResult.steps[]` 的 explicit skip/未执行缺口保持 testing-owned FAIL、零自动 coding
  ...
  绑定已执行且有冻结 `StepResult(assertion + assertion_mismatch)` 的机器证据；旧
  `p0_skip_waiver` receipt 只读且不改变 verdict，P0 未执行不能降级为 WARN。通过率必须
  双口径（skip 计入分母），存在 P0 skip 时结论不得无条件「达标」；不再有
  `await_human_p0_skip` 首触求人 halt。
- **P0 native 证据对账**：Hylyre `0.4.0+` 且 trace `schema_version=0.3-p0` 时，
  `CaseResult.execution=completed`、`verification=passed`、`evidence=complete` 只是必要条件；
  每个 checkpoint 的 required 元素必须映射到同 index 的 `role=assertion,status=passed`
  presence StepResult，forbidden 元素必须映射到 `wait_gone` 的 absence StepResult
  （`observed_present=false,candidate_count=0`）。action-only、inconclusive、incomplete、旧
...
- **失败归因与 legacy**：只消费 StepResult 的 `failure_kind + failure_code`；assertion mismatch
  且 StepResult `status=failed`、非 `evidence.executed=false` 才回 coding，selector 回 testing
