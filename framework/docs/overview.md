...
| **Stop hook 逃生阀（弱模型死锁）** | 未闭环阶段反复"继续"→ 空回复死循环 | `state_machine.max_consecutive_blocks`（默认 3）；连续 N 次「同一组 blocker + harness 未重跑 + 零进展」后 hook `exit 0` 放行 + 二选一提示，交还用户；放行不改 verdict/receipt、不绕过闭环；与 goal 模式 `shouldHaltNoProgress` 同语义 |
...
│   ├── docs/                         对外讲解 + 演进材料（本目录）
│   ├── skills/                       生成层：9 篇阶段 Skill 正文（framework-init · catalog-bootstrap+glossary · spec/plan/coding/code-review/business-ut/device-testing）
...
# Step 1–6：feature 流水线（/spec → /plan → /coding → /code-review → /business-ut → /device-testing）
/spec <feature 描述>
/plan
/coding
/code-review
