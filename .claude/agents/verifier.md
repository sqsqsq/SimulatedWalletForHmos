---
name: verifier
description: 独立的阶段产物语义审查员。当父 agent 完成某阶段（PRD / design / coding / review / ut / testing）产物并通过脚本 Harness 检查后，使用本子 agent 执行对应的 `harness/prompts/verify-<phase>.md` 语义级验证，避免"自己验自己"的偏差。传入参数：feature, phase, 以及 check-*.ts 的结构报告路径。
...
1. 读取 `harness/prompts/verify-<phase>.md` 作为 prompt 模板。
...
- （可选）`trace_dir`: `harness/reports/<feature>/<timestamp>/<model>-<phase>/`

## 工作流

1. **读取规则**：`specs/phase-rules/<phase>-rules.yaml`
2. **读取 prompt 模板**：`harness/prompts/verify-<phase>.md`
3. **读取待审产物**（按 phase）：
