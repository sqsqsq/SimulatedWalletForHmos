...
> `harness/reports/<feature>/<timestamp>/<model>-<phase>/gap-notes.md`
>
...
- **建议改进**（必填，可执行）：
  ...
  - [ ] 修改 `specs/phase-rules/<phase>-rules.yaml` 新增规则：
  - [ ] 修改 `harness/scripts/check-<phase>.ts` 新增自动检查：
  - [ ] 修改 `skills/3-coding/reference/arkts-pitfalls.md` 增加错例：
  - [ ] 其他：
...
- 例：`skills/3-coding/SKILL.md` 的"逐文件 Lint 门禁"在第 3 个文件出错时及时拦截，避免了级联错误。
- …
...
- harness 报告：`harness/reports/<feature>/<timestamp>/<model>-<phase>/check-<phase>.report.md`
- verifier 报告：`harness/reports/<feature>/<timestamp>/<model>-<phase>/verifier.report.md`
- 关键对话片段（可选，脱敏后贴）：
