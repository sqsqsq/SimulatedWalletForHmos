// ============================================================================
// Design 阶段脚本 Harness — check-design.ts
// ============================================================================
// 读取 specs/phase-rules/design-rules.yaml + doc/features/{feature}/design.md
...
function checkPrdMappingTable(ctx: CheckContext, design: string): CheckResult[] {
  const section = getSectionContent(design, 'PRD 功能映射表');
  if (!section) {
    return [{ id: 'prd_mapping_table', category: 'structure', description: ruleDesc(ctx, 'structure_checks', 'prd_mapping_table'), severity: 'BLOCKER', status: 'FAIL', details: '未找到「PRD 功能映射表」章节。' }];
  }

  const tables = extractTables(section);
  if (tables.length === 0) {
    return [{ id: 'prd_mapping_table', category: 'structure', description: ruleDesc(ctx, 'structure_checks', 'prd_mapping_table'), severity: 'BLOCKER', status: 'FAIL', details: '「PRD 功能映射表」中未找到表格。' }];
  }

  const table = tables[0];
  const required = ['PRD 编号', '功能名称', '优先级', '实现模块', '关键文件'];
  const missing: string[] = [];
  for (const req of required) {
    const alternatives = req === 'PRD 编号' ? ['PRD 编号', 'PRD 功能编号'] : [req];
    if (!table.headers.some(h => alternatives.some(alt => h.includes(alt)))) missing.push(req);
  }

  if (missing.length > 0) {
    return [{ id: 'prd_mapping_table', category: 'structure', description: ruleDesc(ctx, 'structure_checks', 'prd_mapping_table'), severity: 'BLOCKER', status: 'FAIL', details: `PRD 映射表缺少列：${missing.join('、')}。实际表头：${table.headers.join('、')}` }];
  }

  return [{ id: 'prd_mapping_table', category: 'structure', description: ruleDesc(ctx, 'structure_checks', 'prd_mapping_table'), severity: 'BLOCKER', status: 'PASS', details: `PRD 功能映射表包含 ${table.rows.length} 行，表头列齐全。` }];
}
...
function checkMetadataHeader(ctx: CheckContext, design: string): CheckResult[] {
  ...
  const required = ['模块标识', '对应 PRD', '版本', '状态'];
...
function checkPrdCoverage(
  ...
  if (!prd) {
    return [{ id: checkId, category: 'traceability', description: ruleDesc(ctx, 'traceability_checks', checkId), severity: 'BLOCKER', status: 'SKIP', details: `PRD.md 不存在，无法验证 ${priority} 覆盖率。` }];
  }
  ...
  if (featureTables.length === 0) {
    return [{ id: checkId, category: 'traceability', description: ruleDesc(ctx, 'traceability_checks', checkId), severity: 'BLOCKER', status: 'SKIP', details: 'PRD 功能清单无表格。' }];
  }
  ...
  if (targetIds.length === 0) {
    return [{ id: checkId, category: 'traceability', description: ruleDesc(ctx, 'traceability_checks', checkId), severity: 'BLOCKER', status: 'SKIP', details: `PRD 中无 ${priority} 功能。` }];
  }

  const mappingSection = getSectionContent(design, 'PRD 功能映射表');
  if (!mappingSection) {
    return [{ id: checkId, category: 'traceability', description: ruleDesc(ctx, 'traceability_checks', checkId), severity: 'BLOCKER', status: 'FAIL', details: `缺少 PRD 功能映射表，无法验证 ${priority} 覆盖率。` }];
  }
  ...
  const rawIds = getColumnValues(mappingTables[0], 'PRD 编号').length > 0
    ? getColumnValues(mappingTables[0], 'PRD 编号')
    : getColumnValues(mappingTables[0], 'PRD 功能编号');
  ...
  return [{
    ...
    suggestion: `请在 PRD 功能映射表中添加缺失的 ${priority} 功能行。`,
  }];
}
...
function checkDesignToArchitecture(ctx: CheckContext, design: string): CheckResult[] {
  const archPath = path.join(ctx.projectRoot, 'doc', 'architecture.md');
  if (!fs.existsSync(archPath)) {
    return [{ id: 'design_to_architecture', category: 'traceability', description: ruleDesc(ctx, 'traceability_checks', 'design_to_architecture'), severity: 'MAJOR', status: 'SKIP', details: 'doc/architecture.md 不存在，跳过。' }];
  }
  ...
  if (unregistered.length === 0) {
    return [{ id: 'design_to_architecture', category: 'traceability', description: ruleDesc(ctx, 'traceability_checks', 'design_to_architecture'), severity: 'MAJOR', status: 'PASS', details: `全部 ${moduleNames.length} 个模块在 architecture.md 中均有记录。` }];
  }
  ...
  return [{
    ...
    severity: 'MAJOR', status: 'WARN',
    details: `${unregistered.length} 个模块未在 architecture.md 中找到：${unregistered.join('、')}`,
    affected_files: ['doc/architecture.md'],
    suggestion: '请在 architecture.md 中注册所有新增/修改的模块。',
  }];
}
  ...
  async check(ctx: CheckContext): Promise<CheckResult[]> {
    ...
    if (!design) {
      return [{
        ...
        description: `doc/features/${ctx.feature}/design.md 不存在`,
        severity: 'BLOCKER', status: 'FAIL',
        details: `设计文件 doc/features/${ctx.feature}/design.md 不存在，无法进行任何检查。`,
        affected_files: [`doc/features/${ctx.feature}/design.md`],
      }];
    }

    const prd = loadDoc(ctx, 'PRD.md');
    ...
    results.push(...safeRun(() => checkPrdMappingTable(ctx, design), 'prd_mapping_table'));
