...
export function loadGlossary(
  ...
  const fullPath = path.join(projectRoot, GLOSSARY_RELATIVE_PATH);

  if (!fs.existsSync(fullPath)) {
    return { ok: false, error: { kind: 'file_not_found', path: GLOSSARY_RELATIVE_PATH } };
  }
...
export function describeGlossaryError(err: GlossaryLoadError): string {
  switch (err.kind) {
    case 'file_not_found':
      return `未找到 ${err.path}（请先创建 doc/glossary.yaml）`;
    case 'invalid_yaml':
      return `doc/glossary.yaml YAML 解析失败: ${err.message}`;
    case 'invalid_schema':
      return `doc/glossary.yaml schema 错误: ${err.message}`;
  }
}
