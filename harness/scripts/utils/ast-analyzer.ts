...
export interface ImportInfo {
  ...
  modulePath: string;
  isRelative: boolean;
  /** 按路径推断的来源层（如 'data', 'domain', 'presentation', 'shared'） */
  sourceInternalLayer?: string;
  /** 按路径推断的来源架构层（如 '01-Product', '05-SystemBase'） */
...
const INTERNAL_LAYERS = ['shared', 'data', 'domain', 'presentation'] as const;
type InternalLayer = typeof INTERNAL_LAYERS[number];

const INTERNAL_LAYER_ORDER: Record<InternalLayer, number> = {
  shared: 0,
  data: 1,
  domain: 2,
  presentation: 3,
...
/** 层内依赖：只能 依赖等级 <= 自身的层（即不能向上依赖） */
const FORBIDDEN_INTERNAL_IMPORTS: Record<InternalLayer, InternalLayer[]> = {
...
const ARCH_LAYER_ORDER: Record<string, number> = {
  '01-Product': 1,
  '02-Feature': 2,
  '03-CommonBusiness': 3,
  '04-BusinessBase': 4,
...
export class AstAnalyzer {
  ...
  /** 从文件路径推断模块内部层 (shared/data/domain/presentation) */
  inferInternalLayer(filePath: string): InternalLayer | undefined {
    const normalized = filePath.replace(/\\/g, '/');
    for (const layer of INTERNAL_LAYERS) {
  ...
  private inferArchLayer(modulePath: string, currentFile: string): string | undefined {
    ...
    for (const layer of Object.keys(ARCH_LAYER_ORDER)) {
  ...
  /** 获取文件所属的内部层 */
  getFileInternalLayer(filePath: string): InternalLayer | undefined {
  ...
  checkInternalLayerCompliance(analysis: FileAnalysis): Array<{
    file: string;
    import: ImportInfo;
    fileLayer: InternalLayer;
    importLayer: InternalLayer;
    message: string;
  }> {
    ...
    const forbidden = FORBIDDEN_INTERNAL_IMPORTS[fileLayer];

    for (const imp of analysis.imports) {
      if (!imp.isRelative || !imp.sourceInternalLayer) continue;
      const importLayer = imp.sourceInternalLayer as InternalLayer;
  ...
  checkArchLayerCompliance(analysis: FileAnalysis): Array<{
    ...
    if (!fileArchLayer || !ARCH_LAYER_ORDER[fileArchLayer]) return violations;

    const fileOrder = ARCH_LAYER_ORDER[fileArchLayer];

    for (const imp of analysis.imports) {
      const importArchLayer = imp.sourceArchLayer;
      if (!importArchLayer || !ARCH_LAYER_ORDER[importArchLayer]) continue;
      if (importArchLayer === fileArchLayer) continue;

      const importOrder = ARCH_LAYER_ORDER[importArchLayer];
      if (importOrder < fileOrder) {
        violations.push({
          file: analysis.filePath,
          import: imp,
          fileArchLayer,
