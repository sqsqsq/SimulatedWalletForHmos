...
//
// 消费者:
//   - harness/scripts/check-prd.ts (scope_matches_catalog / terminology_mapping_table)
//   - harness/scripts/check-design.ts (交叉校验 scope 一致性)
// ============================================================================
...
export function loadCatalog(
  ...
  const fullPath = path.join(projectRoot, CATALOG_RELATIVE_PATH);

  if (!fs.existsSync(fullPath)) {
    return { ok: false, error: { kind: 'file_not_found', path: CATALOG_RELATIVE_PATH } };
  }
...
export function describeCatalogError(err: CatalogLoadError): string {
  switch (err.kind) {
    case 'file_not_found':
      return `未找到 ${err.path}（请先创建 doc/module-catalog.yaml）`;
    case 'invalid_yaml':
      return `doc/module-catalog.yaml YAML 解析失败: ${err.message}`;
    case 'invalid_schema':
      return `doc/module-catalog.yaml schema 错误: ${err.message}`;
  }
}
