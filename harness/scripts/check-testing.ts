...
// ============================================================================
// 读取 specs/phase-rules/testing-rules.yaml + specs/features/{feature}/
// 执行确定性的静态验证。
//
...
function loadDoc(ctx: CheckContext, docName: string): string | null {
  const docPath = path.join(ctx.projectRoot, 'doc', 'features', ctx.feature, docName);
  ...
  async check(ctx: CheckContext): Promise<CheckResult[]> {
    ...
    if (!plan && !report) {
      return [{
        ...
        severity: 'BLOCKER',
        status: 'FAIL',
        details: `未找到 doc/features/${ctx.feature}/test-plan.md 和 test-report.md。测试阶段至少需要测试计划。`,
