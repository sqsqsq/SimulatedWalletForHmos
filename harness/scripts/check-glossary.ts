...
function checkSchemaVersionPresent(ctx: CheckContext, glossary: Glossary): CheckResult[] {
  ...
  return [{
    ...
    suggestion: '在 doc/glossary.yaml 顶部添加 `schema_version: "1.0"`',
    affected_files: ['doc/glossary.yaml'],
  }];
}
...
function checkTermRequiredFields(ctx: CheckContext, glossary: Glossary): CheckResult[] {
  ...
  return [{
    ...
    suggestion: '对照 skills/0-catalog-bootstrap/templates/glossary-term-template.yaml 补齐。',
    affected_files: ['doc/glossary.yaml'],
  }];
}
...
function checkOwnerLayerValueValid(ctx: CheckContext, glossary: Glossary): CheckResult[] {
  const invalid = glossary.terms
    .filter(t => !ALLOWED_LAYERS.includes(t.owner_layer))
  ...
  return [{
    ...
    suggestion: `owner_layer 必须是：${ALLOWED_LAYERS.join(' / ')}`,
    affected_files: ['doc/glossary.yaml'],
  }];
}
...
function checkCanonicalModuleExistsInCatalog(
  ...
  return [{
    ...
    affected_files: ['doc/glossary.yaml', 'doc/module-catalog.yaml'],
  }];
}
  ...
  async check(ctx: CheckContext): Promise<CheckResult[]> {
    ...
    if (!gResult.ok) {
      return [{
        ...
        description: 'doc/glossary.yaml 加载失败',
        severity: 'BLOCKER', status: 'FAIL',
    ...
    if (!cResult.ok) {
      return [{
        ...
        description: 'doc/module-catalog.yaml 加载失败，无法完成交叉校验',
        severity: 'BLOCKER', status: 'FAIL',
        details: describeCatalogError(cResult.error),
        affected_files: ['doc/module-catalog.yaml', 'doc/glossary.yaml'],
