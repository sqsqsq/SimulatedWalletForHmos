...
// ============================================================================
// 读取 specs/phase-rules/review-rules.yaml + doc/features/{feature}/review-report.md
// 执行确定性的结构 / 追溯验证。
//
  ...
  async check(ctx: CheckContext): Promise<CheckResult[]> {
    ...
    if (!report) {
      return [{
        ...
        description: `doc/features/${ctx.feature}/review-report.md 不存在`,
        severity: 'BLOCKER', status: 'FAIL',
        details: `审查报告 doc/features/${ctx.feature}/review-report.md 不存在，无法进行任何检查。`,
        affected_files: [`doc/features/${ctx.feature}/review-report.md`],
      }];
    }
