...
| `node doc/extensions/hooks/plan/author.mjs --feature <名>` 的输出（本次任务包，动笔前取一次） | 命中规约的原文入口、项目事实入口、spec §9.4 原文与它的统计点逐条、每个统计点要回答的几问。`context-exploration` 的 `key_inputs_read` 要含本页——任务包是它的展开 |
...
spec §9.4 列了统计点时，「服务层接口定义」章下写「埋点」小节（形状见模板，统计点与要问的几问由任务包逐条列出）：不是把 §9.4 再抄一遍，是对**每个统计点**回答责任方法、本端取得结果的调用或查询、去重与耗时、怎么验证；项目知识定义了身份、编码、分类、外部错误码的，再给实际值，或写清缺哪一段、由谁按什么规则定；缺封装或协议支撑的列为建设项。
...
- spec §9.4 的统计点在埋点小节没有对应行；某行没写责任方法，或写的方法在 `contracts.yaml` 的 `interfaces[].methods[]` 里找不到；spec 把某条规约落在某个统计点上，而该点的责任方法没挂这条 `must`。
