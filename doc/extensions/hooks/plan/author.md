...
spec §9.4 列了统计点时，「服务层接口定义」章下写「埋点」小节（形状见模板，指标与统计点由任务包列出）。承接 Spec 已定的指标、点位与业务结果，按激活的统计与上报项目知识里的上报实现（新值按其编号分配），逐个业务结果落实：责任方法与分支、身份与字段的具体取值或候选及来源、同次尝试与重试边界、每种结果的预期事件。共同定义集中写在本节，结果行引用定义；方法、参数模型、返回类型与要实现的文件写进契约。给定规则能产生具体候选时本阶段给出候选；尚缺依据时点明缺失项与受影响的设计，提交裁决。一个统计点有几种适用结果就写几行。「待登记」只表示值已设计、尚未写入常量文件；缺项目分配依据时写清缺哪一项、影响哪几行，不把整个点位留给编码。建设新封装是设计动作，给接口与责任，公共封装不代替业务方法承担调用责任。写完一行，实现者要能回答：在哪个分支、取哪些值、调用什么、看到怎样的事件。
