...
- 埋点写在「服务层接口定义」章下的一个小节，按 spec §9.4 的流程逐统计点写：结果由谁、凭什么调用、回调、查询或通知得到；身份与参数；各实际适用结果的含义与编码来源；复用还是新增；怎么验证。已有定义写出实际值，没有分配依据的列为待决，不写「同上登记」「在某枚举定义」代替设计；缺封装或协议支撑的列为本需求的建设项。观察来源与业务状态的要求取自本需求命中的已激活知识，义务挂法见模板样例。
