...
- spec §9.4 缺埋点一节或没有点位表；某个统计点在埋点小节没有结果行；某条结果行没写责任方法，或写的方法在 `contracts.yaml` 的 `interfaces[].methods[]` 里找不到；spec 把某条规约落在某个统计点上，而该点结果行的责任方法没挂这条 `must`。
