...
spec §9.4 列了统计点时，「服务层接口定义」章下写「埋点」小节（形状见模板，指标与统计点由任务包列出）。先查真实接口与项目知识，再逐个业务结果落实。读取 Spec 已定的指标、点位与业务结果。对每个结果，先定位能够确定它的接口返回、回调或用户动作，再在该责任方法中决定上报时机。查项目知识与已有定义，形成身份、结果及必要参数的具体取值或取值表达式，并说明来源。共同定义集中写在本节，结果行引用定义。结合一次实际执行说明去重、重试、取消和耗时边界。将方法、参数模型、返回类型与需要实现的文件写入契约，再列每种适用结果对应的原始事件验证。给定规则能产生具体候选时，本阶段给出候选；尚缺依据时点明缺失项与受影响的设计，提交相应裁决。一个统计点有几种适用结果就写几行。「待登记」只表示值已设计、尚未写入常量文件；缺项目分配依据时写清缺哪一项、影响哪几行，不把整个点位留给编码。建设新封装是设计动作，给接口与责任，公共封装不代替业务方法承担调用责任。写完一行，实现者要能回答：在哪个分支、取哪些值、调用什么、看到怎样的事件。
