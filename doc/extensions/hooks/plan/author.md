...
不重做已定的适用性与选型——coding 按已选模式读下篇与必要 facts，其余读**你挂在契约实体上的 `must`** 与 `files` 的 `pattern` / `role`。挂错或没挂，下游零注入。
...
| 上游 `spec/spec.md` 的 §10、§11 | §10 每条命中条目在本阶段都要有实体扛着，它的强制力与验法两列定落点要什么证据；§11 的候选是选型的出发点 |
...
- 一条 `must` 就是一处落点，`verify` 说这一处的证据由谁取，按该规约验证列声明的执行体定：

  | 执行体 | 这处落点的 `verify` |
  |---|---|
  | 含「实机」 | `ut` / `device` / `both`——一处标 `review` 就是这一处没证据 |
  | 只有「模型」（或加「构建」） | `review` 即可 |
  | 「人工」 | 不挂 `must`（评审动作） |
