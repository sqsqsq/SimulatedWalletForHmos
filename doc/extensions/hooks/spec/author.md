...
- **命中**：写 `requirement`——**列表，一条要求一句**。把编号遮住，这一句还能指导编码吗？
  换一个需求照样成立，那是复述不是要求。几件事就写几条，别用分号串成一段：
  §10 逐条成行，一格里堆一百多字，读者要数分号才分得出这是几件事。
  再写**落点，二选一**：`contract` 引 §9 里登记的接口/存储键/配置项名（脚本会验它真在 §9），
  或 `impact` 写实际影响对象（RTL、图标、文案、翻译这类本来就没有 §9 实体——
  但要点名，「页面」「资源」这种泛称审查会退回）。两个都不写、或都写，都会被拦。
...
判定的推演与决策论证是工作底稿，**不进 spec**——它是交给代码的要求说明书。
规约条目的完整逐条回显在归档件附录，也不进 spec。
