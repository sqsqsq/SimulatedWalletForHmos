...
export default async function preVerifier(ctx) {
  ...
  const fragment = [
    ...
    '',
    '**裁决表必须整张出现在你的最后一条回复里**，固定表头 `| 编号 | 裁决 | 证据 |`，',
    '一行一条，编号照上表原样写。不要分段发、不要放在中间某条消息里、不要说「见上文」——',
    '报告文件不由你写：SubagentStop 钩子只取你的**终态消息**生成 `verifier.report.<subject>.json`，',
    '门禁读的是那份 JSON 的结论正文。没进终态消息的表，落盘时就不存在，缺行即 BLOCKER。',
    '只写在 YAML 输出里同样不算——那份输出不落盘。',
    '',
