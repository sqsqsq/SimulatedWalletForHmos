...
模板枚举位于 `CommFunc/src/main/ets/shared/ha/WalletHAEventID.ets`，当前有 `Wallet_COMMON`。当前仓尚无业务流程、步骤和内码登记；首次使用时在实现设计中明确登记位置及分配依据，不把示例值当已分配值。

身份定义不是一次执行的流水号。已有定义按语义复用，新的具体结果扩展内码，不为同一步骤的不同结果复制身份常量。

## 6. 使用参考

以下以可重试的短信验证说明关系，不规定所有业务都采用相同步骤。整个验证步骤及两个子点共用步骤身份，内码子节点段区分统计对象。

| 统计对象 | 子节点段示例 | 适用结果示例 |
|---|---|---|
| 短信验证步骤 | 00 | 验证通过为 STEP_SUCCESS；明确终止验证为主动取消；无法继续为普通失败 |
| 发送验证码 | 01 | 发送成功为 STEP_SUCCESS；发送失败为普通失败 |
| 校验验证码 | 02 | 校验通过为 STEP_SUCCESS；校验未通过为普通失败 |
