...
| 模式概念 | 在设计里是什么 |
|---|---|
| 交互管理器持有者 | 构建动作表并触发的类，带角色名的实现文件与它的接口；是正常设计实体，不另设旁账 |
| 动作枚举 | 页内执行步骤，属于实现接口，不是业务状态 |
| 用户可触发的动作 | 用户在页面上能做的事，以及它调用的业务方法 |
| 一次完整操作序列 | 用户视角从进入到结果的一串操作 |
| 对外业务状态 | 页面对外可见的阶段（加载中、已提交、失败） |
...
4. 用户动作、完整操作序列和对外业务状态交给业务用例，管理器与动作接口交给实现契约。
...
| 对象 | 完成判断 |
|---|---|
| 选型设计 | 页面单元、关键事实与采用理由写清；实例名唯一；动作与等待点进了正式设计 |
| 实现 | 动作表覆盖枚举全部键；自动后继与 `nextNodes` 一致；等待用户的 operator 无返回值；最小编译通过 |
| 审查关注 | 页面只通过 `doOperator` 发起业务动作；不存在回调间直接串联；跳转、上报和业务提交各有明确所有者 |
