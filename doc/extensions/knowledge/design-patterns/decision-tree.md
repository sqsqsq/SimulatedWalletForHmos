...
采用之后，设计里要分清下面几样东西各是谁、承担什么；它们都是正常的设计实体，不另设旁账：

| 模式概念 | 在设计里是什么 |
|---|---|
| 树的持有者 | 构建并启动这棵树的类，带角色名的实现文件与它的接口 |
| 节点枚举 | 内部执行步骤，属于实现接口，不是业务状态 |
| 对外业务状态 | 用户或界面能观察到的阶段，界面订阅它、业务用例断言它 |
| 一条完整路径 | 用户视角的一次业务往返：从哪进、经过哪些对外状态、到哪个结果 |
| 取数与落库的边界 | 节点里读写数据的范围 |

**边界判据**：对外业务状态是界面订阅、业务用例断言的东西。决策树的内部节点（如「构造指令」「校验写入」）不是业务状态，
当成业务状态会让业务用例断言实现细节，之后重构树结构就被误判成业务行为变化。只有某个节点本来就是对外发布的状态时，两处才同时出现。
...
4. 把内部节点与对外业务状态分开交给契约：节点进实现接口，对外状态与完整路径进业务用例，避免测试锁死实现步骤。
5. 若与页面交互编排组合，写清每个模式解决哪个独立问题、上下文所有者、交接接口、用户等待点与失败收敛者。
...
| 对象 | 完成判断 |
|---|---|
| 选型设计 | 适用单元、关键事实与采用理由写清；实例名唯一；节点不冒充业务状态；每条 P0/P1 分支有入口和终点 |
| 实现 | 节点表覆盖枚举全部键；唯一 root；每个实际返回值都在 `nextNodes`；终点是 leaf；最小编译通过 |
| 审查关注 | 调用方无第二套 if/else 选路；节点不直接跳页面或持有仓储单例；SDK 行为事实与工程约定没有混写 |
