...
3. **取证并写**：
   ...
   - 按 protocol 写：frontmatter 的 `applies_when` 写清何时读、回答什么；正文正向陈述有什么、怎么用；多步推导的写应用步骤、一个短例与怎样算用对了；
   - 同一内容只在一份里写；编码规范、架构配置已经写的不再写；Demo 知识只参考组织方式，内容以本仓为准；
...
目标是一个非钱包的后台服务仓。它有一份 `knowledge/facts/services.md`，`applies_when` 只写了「always」，正文列着通知封装 `Notifier.send` 与重试策略。
