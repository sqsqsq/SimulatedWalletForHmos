...
> 读者：S2 导入材料的主 agent。时机：inbox 里有文件时完整读一遍；空收件箱直接去初析，不必读本文。
...
|---|---|---|---|
| `RR` | 要解决的问题、给谁用、业务规则是什么——**用户与业务视角**（问题单、产品需求、运营规则都在此） | 背景、范围、业务流程、交付与上线各章按 PRD 读 | `RR/prd.md` |
...
并进正文会用草稿盖掉定稿，而人要的只是图。**这一判断由你按材料做，不为它再问人一次**——
人已经说过「补图」了。反过来，正文确实更新更全时照常归 RR / SR / AR，覆盖是对的。
