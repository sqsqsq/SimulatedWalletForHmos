...
**`decisions.json` 随 story 登记冻结，登记之后只读。** 评审引起的状态变化只记进
`AR/review-disposition.json`（§2），不回写冻结件——它记的是这份 story 据以成文的依据，
改了它，产物与依据就对不上；门禁会拦，而那是拦对了。
...
`AR/review.md` 的**人工区**归人所有——`story-build.mjs build` 不再重建它，只备份。
人可以自由编辑本地文件，不必担心被下一次 build 冲掉。
**结构归渲染器**：章节骨架、议题条目、三级形态由 `build` 生成并保持。

### 回传意见写到哪

`/story review` 拉回的意见**写进 review.md 本身**，不另建第二份输入文件——
下游只读 `AR/review.md` 这一份，多一份就多一个真源。逐条落位：

| 回传形态 | 写到哪 |
|---|---|
| 针对某条议题的表态 | 该议题的「审核结果：」行后面 |
| 没有对应议题的意见 | 「其他意见」一章（`<!-- freeform-zone -->` 区内） |
| 评审人给的是**整文批注版** | **不要拿它整份覆盖 review.md**——把意见逐条摘回上面两处；原批注件由既有备份机制留在 `.review-backup`，需要核原文时去那里看 |

整份覆盖会把渲染器维护的三级结构冲掉——发生过一次：正本变成了批注版，
结构完整的那一版退到了备份里。摘回是判读，判读结果照 §2 落台账。
