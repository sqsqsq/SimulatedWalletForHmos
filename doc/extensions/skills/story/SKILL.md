...
```
S1 取材        →  拉三套材料、建工作区骨架
S2 导入+初析   →  列 inbox → 导入 →（有会议材料时）读会 → 材料盘点 →（材料确认后）需求分析
...
**先做完对应产物，再问对应问题**：盘点完材料才问还缺什么，分析完范围才问范围怎么定
...
做到 spec 闭环并通过交付门。在流程中途再问一遍，是把一个已经有主的问题重问一次，
...
| # | 停在哪 | 什么时候停 |
|---|---|---|
| 1 | **材料关卡**（`material_scope`） | `init` 之后**必停**，无论材料看起来齐不齐 |
| 2 | **范围关卡**（`scope_decision` 及其追问 `split_carrier`） | **必停**。切法与承载哪份是同一次对话里的追问，算这一处 |

**两处都是无条件的。** 「要不要停」不由判断材料齐不齐、范围有没有变来决定——
那等于把停等的开关交给被停的那一方。你的判断只进**选项推荐**，定不定由人。
脚本层也这么拦：三级关卡的决策**只认人签**，`decide` 没有代签参数，记录里一律写 human。

**这一级请人陈述的是事实**：料放进去了，或者现有材料就是全部。够不够由你盘点、
由人定——**文件到了不等于内容够了**，机器不判这件事。

**导入之后再问，问的是「还缺什么」**。人回哪一项，收件箱里的原件都会被导入——
文件已经在盘上，导入是脚本的活，`status` 会直接给出导入命令，不用再问一次
「放好了吗」。导完拿新材料**重新盘点**（那是你的活）：不缺了就直接进需求分析
（`status` 把 `next` 给成 `run_analysis`）；还缺就把**剩下的**缺口写进选项侧车
（`gate` 写 `material_scope`）再停一次。上一轮缺的没补上照样可以再问——
问的是还缺什么，不是够不够。所以这一处停几次由缺口决定，不由轮次决定。
...
```bash
node doc/extensions/skills/story/scripts/core/story-build.mjs check --deliver --feature <AR>   # ① 交付门：框架回执 + 读者审查的实际结论
...
归档件保持快照不等于当前产物不改：人的新决定照常改进 spec 与下游，差异记进台账，下游取到当前依据才算闭合。
...
（写「材料已放进 `<完整路径>`，请导入」——它陈述的是一件脚本当场就要去核的事实；
写成「补充材料后继续」的话，读起来像「我打算去补」，而目录此刻就要被检查）。
