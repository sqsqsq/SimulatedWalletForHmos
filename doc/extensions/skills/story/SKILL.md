...
**第二层不另造一套授权。** framework 的推进策略解析的是**用户消息**，扩展写什么都
改变不了它的判定结果。所以用户的目标越过 spec 时（例如「做到 plan」「全链路交付」），
在 S4 收口那一刻一次讲清：请他在下一条消息里把范围说出来，或者走 framework 的阶段
推进确认点一次拿到范围授权。**一次问代替逐段问**——不要每闭环一个阶段问一次，
也不要自己替 framework 判定授权。
