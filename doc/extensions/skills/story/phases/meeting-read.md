...
- **归属**：判得准属于本需求的写 `ours`；判得准不属于的写 `not_ours`，只记不消费；
  判不准写 `unclear`，并写 `question`；
- **问不问**：下列任一成立就写 `question`，这一轮要人定：
  1. 结论改变本单的范围、接口或验收；
  2. 结论与已归档文档或人已做的决定冲突；
  3. 判不准归属；
  4. 转写存疑且影响结论。
  会上说定了，只要满足其中一条，仍要人点头；
- 有 `question` 时至少给一个真实选项（`key` + `label`），`recommend` 若给必须是其中一个 key，
  并在 `recommend_why` 写一句理由；简单确认不硬造两个方案。没有 `question` 就不写选项与 `question` 键。

**尺度**：会上明确收口的写成已定，判据是记录里那句话，引得出来；没有那句话的写成未决，不替人定。
文档与会议冲突时写明冲突与你的判断，由人裁决。

## 四、摆给人那一轮

会议是需求材料的一种，材料关卡已经问过，这里不再问材料。会议判断里有带 `question` 的话题时，
`status` 给出 `await_gate:meeting` 与问法 `ask`（每个要问的话题一组选项），停这一次；没有要问的就不停，直接写当前会议结果。
摆之前回 `raw.md` 自查一遍——话题有没有漏、原话指得对不对、与已有决定的冲突有没有说出来。停等消息里有三样：

1. 要问的话题：`ask.block` 原样，每组选项前是那个话题的问题；
2. 不问的话题：每个一句去向（按会议采纳了什么、不属于本单、已被后续决定替代）；
3. 参会人角色：你判出的角色一行，请人确认。

人一轮答完，逐话题记：

```
python doc/extensions/skills/story/scripts/core/story_flow.py decide --feature <AR> --gate meeting --meeting <主名>@<版本> --item <话题 id> --ask <ask_id> --reply "<人的原话>" [--chosen <编号>]
```

人给的是选项之外的答案时，先把它补进会议判断里那个话题的 `options`（新 `key` + 他说的那个做法），重跑 `status`
取新的问法，再记——**记的必须是人真说的那一项**，不能替他选推荐项。人选「保持待定」的话题就是未决：
当前会议结果里写成未决与边界，spec 与决策登记里登记为 `open` 议题，产物写「待定：谁定、等什么」。
...
- **暂缓就是未决**：还没定的写成未决与边界，不写任何一种实现，也不写成本期不做。
