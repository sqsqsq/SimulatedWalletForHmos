...
- `AR/review.md` **由 `AR/story-src/decisions.json` 渲染，不手写**：AI 只负责把开放点收齐并登记
  ...
  **表态位只能由评审人填**：三态勾选与它下面那一行说明（修改意见 / 暂缓原因），
  你一个字都不写。`story-build build` 重渲染时只重算机器区，人工区逐字节保留。
  代填表态即 BLOCKER——归档件上「已同意」若不是评审人自己勾的，它就不可信。
...
是首版草稿，请开发按其中议题逐条审核并勾选表态
→ `/story archive` 送审 `AR/story.md` 与 `AR/review.md`。
AI 不代填表态、不动人工区。表态完成度看的是**每个议题块三态至少勾了一个**；
未勾的议题在进 plan 时逐条列出。「进入 plan」只能作为其后的选项列出，
并注明「建议先完成评审与归档」。
