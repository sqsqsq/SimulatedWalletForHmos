...
| **目标** | 通过 AI 自动生成业务级端到端 UT，覆盖关键业务流程，兼具拦截能力与执行效率 |
...
这是整个系统的核心数据结构。使用 YAML 格式定义，相比 JSON 更易于人工阅读和编辑，且天然支持注释：
...
nodes:
  - id: n1
    ...
    source:
      file: pages/CardPage.ets
      function: onCardOpenClicked
    next: [n2]
  ...
  - id: n2
    ...
    source:
      file: services/CardService.ets
      function: fetchCardParams
  ...
  - id: n5
    ...
    assertions:
      - type: state_check
        target: CardViewModel.cardStatus
        expected: ACTIVATED
...
- `dag-schema.yaml` — DAG 规范定义文件（YAML 格式）
...
- **日志埋点方案**：在关键节点增加结构化日志，记录 `{timestamp, module, function, event_type, params}`
...
```
skills/
└── business-ut-generator/
    ├── SKILL.md              # Skill 入口描述文件
    ├── templates/
    │   ├── dag-schema.yaml   # DAG 规范模板（YAML 格式）
...
| MCP Tool | 功能 | 输入 | 输出 |
|----------|------|------|------|
| `build_dag` | 从代码构建 DAG | 入口函数路径、日志文件(可选) | DAG YAML |
...
| DAG 格式 | YAML + JSON Schema 校验 | 人类可读性强，支持注释，易于手工编辑和 AI 生成 |
